...
| **ZAOstock** | `@ZAOstockTeamBot` | `bot/src/index.ts` (root) | Festival team coordination — **graduating** with ZAOstock spinout. Proactive pings (digests, devops alerts) DM Zaal, not the group | systemd `zaostock-bot` (from `~/zaostock-bot` snapshot) | ✅ heartbeat + lifecycle |
...
- 10-bot branded fleet (Magnetiq / Research / WaveWarZ / POIDH as own bots) — Magnetiq/AttaBotty code **deleted** 2026-06-17 (`bot/src/teams/` removed)
- FISHBOWLZ (killed 2026-05-04, graduated to `bettercallzaal/fishbowlz`) — code dirs already gone; only the graduation-pointer skill (`.claude/skills/fishbowlz/`) remains (intentional). Stray ref left: the `fishbowlz` entry in `src/components/admin/agents/constants.ts` (part of the broader OpenClaw-squad dashboard — fold into that cleanup, not piecemeal)

## Infra

- **Bots VPS** `zaal@31.97.148.88` — systemd `--user` units: `zoe-bot`, `zao-devz-stack`, `zaostock-bot` (+ `zao-fleet-agent` staged/off). Each an independent grammy long-poll loop; no central dispatcher. (`zao-team-bots` retired — code deleted 2026-06-17; remove any leftover systemd unit on the box.)
...
4. **Decommissioned code lingering** — Magnetiq/AttaBotty deleted 2026-06-17. Remaining: the `fishbowlz` entry in the OpenClaw-squad admin dashboard constants (fold into a full squad-dashboard cleanup).
