 ...
 * uses the cheaper fast model and we escalate to the full model on retry.
 * Many fixes are docs/typo/small-refactor and don't need Opus to write
 * correctly. If Critic rejects (<70), the next attempt has feedback to
 * reason about, so the bigger model earns its keep.
 *
 * When HERMES_ROUTING=off (default), every attempt uses HERMES_FIXER_MODEL
 * (opus) - identical to pre-Sprint-1 behavior.
 */
function selectFixerModel(attemptNumber: number): string {
  if (!HERMES_ROUTING_ENABLED) return HERMES_FIXER_MODEL;
  return attemptNumber === 1 ? HERMES_FIXER_FAST_MODEL : HERMES_FIXER_MODEL;
}
...
export async function runFixer(input: FixerInput): Promise<FixerOutput> {
  ...
  const fixerModel = selectFixerModel(input.attemptNumber);
  ...
  return {
    ...
    prBody: `${summary.prBody}\n\n---\n_Generated by Hermes Stock-Coder via Claude Code CLI (Max plan auth, model: ${fixerModel}). Verified by Hermes-Stock Critic before this PR opened._\n\n_Rationale:_ ${summary.rationale}`,
    inputTokens: result.inputTokens,
    outputTokens: result.outputTokens,
    modelUsed: fixerModel,
  };
}
