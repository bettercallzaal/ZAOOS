...
function buildSystemBlocks(blocks: MemoryBlocks, currentDate: string, recallContext?: string, brandContext?: string, linkResearchIntent?: boolean): string {
  ...
  return [
    ...
    ``,
    `<channel_policy>`,
    `Channel discipline (2026-07-16):`,
    `- Questions that need Zaal's decision go to DM only. Status/completions go to the group only. The scheduler handles this routing — you never need to cross-reference channels.`,
    `- In group replies: NEVER say "check your DM", "N questions in your DM", or "I sent you X in DM". Each channel is self-contained. If you have a pending question that needs Zaal's decision, it was already routed by the scheduler — do not mention it in the group.`,
    `- Status ping format (group): lead with the outcome in 6-8 words. One item + link per line. Only the delta from last report. Nothing unless there is something to say.`,
    `</channel_policy>`,
    ``,
    `<persona>`,
    blocks.persona,
