...
respawn spawn-server  "node $HOME/bin/spawn-server.js 2>&1 | tee $HOME/spawn-server.log"
respawn auth-server   "node $HOME/bin/auth-server.js 2>&1 | tee $HOME/auth-server.log"
