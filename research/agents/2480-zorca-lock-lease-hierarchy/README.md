...
## Updated 2026-09-25 - re-measured against today's live state

Re-verified every claim and every Next Action from the 2026-09-09 version, fresh, sixteen days later. One shipped cleanly, one shipped and then silently broke, one is overtaken by a newer decision, and two never landed:

- **The cheap fix (heartbeat cron) shipped 2026-09-09, then went silently useless on 2026-09-13 and nobody has noticed for 12 days.** `crontab -l` today shows the exact recommended line (`*/15 * * * * ... zorca-lock heartbeat zj`), on schedule. But `~/.zao/zorca-lock-heartbeat.log` reads `not yours; claim first` on every single run back through the log - because `zj` released `zorca-lock` on 2026-09-13 when the top seat moved to `vault` (per `~/.zao/orchestrators.json`'s `_seat_log`), and nobody re-issued a `claim` since. `zorca-lock status` today reads plain `free` - not stale, genuinely unclaimed. **This is the same "inverted alarm" class doc 2478 catalogued the same week: a cron that runs on time, writes to its log every 15 minutes, and does nothing, for 12 straight days, with no distinct failure signal from "working fine."** The Next Action shipped the letter of the fix and missed the point - a heartbeat with nobody claiming the lock first is not a liveness fix, it is a no-op with a green cron entry.
- **The tier recommendation (Key Decision 3-4) was answered, then overtaken three days later by a different shape.** `orchestrators.json`'s `tiers` block did go LIVE on 2026-09-09 ~22:0x almost exactly as this doc recommended (primary=zj, secondary=vault oversees tertiaries). But a *second*, materially different hierarchy (`hierarchy_2026_09_13`) superseded it four days later: **vault** is now the top seat and explicitly **may not code** ("zaovault manaages all the terminals... it shouldnt built it should jsut manage and note things to obsidian"), **zorca** is "the first reviewer of any lane," and zj is now just "the build and talk lane" with "everthing else... under zorca." So the primary/secondary/tertiary vocabulary this doc argued for is not wrong, but the *assignment* of primary=zj this doc measured against is stale - the seat has moved again, confirmed directly by Zaal (not a relay), and this doc's authority table (section 3) needs re-reading against `hierarchy_2026_09_13`, not the 2026-09-09 `orchestrators.json` state quoted below.
- **The lease/fencing port (Key Decision 2, the doc's central engineering recommendation) has NOT shipped, three days past its 2026-09-22 due date.** `git log --oneline -- bin/zorca-lock` in `~/zaal-dotfiles` shows exactly one commit total (2026-08-27, the original copy) - zero commits since this doc was written. The script read today is byte-for-byte the same code quoted below: no `generation`, no fencing token, no `--selftest`, no compare-and-swap. **The central technical claim - a bare JSON file with a TOCTOU race and no fencing - still holds, unchanged, and is now the older of two problems**, since the seat is currently unclaimed rather than raced.
- **The AGENTS.md three-tier vocabulary edit (Key Decision 4) has NOT shipped.** `notes/agent-registry-seed-2026-09-09.md` today: `role` is still free prose ("Domain orchestrator, zao-vault; no longer top seat", "Coding/agent CLI running lane sessions...") on every one of its 63 data rows, not one of `primary`/`secondary`/`tertiary`. `escalation` reads literally `UNMEASURED` on 28 of 63 rows (44%) - down from "at least 40 of 65" but still far from the "zero UNMEASURED for live lanes" bar this doc set.
- **The stale-takeover log line (last Next Action) has NOT shipped, and the underlying convention was never true.** `grep -i "zorca-lock" ~/.zao/orca-board.log` (1.8 MB, 24,499 lines) returns zero matches - `zorca-lock` has never written to that log at all, for a claim or a takeover, contrary to this doc's assumption that "log every send" already covered it.

**Net effect on the central claim:** unchanged and confirmed - the lock is still a bare, unfenced, untested JSON file, and the seat still needs a lease, not a lock. What changed is the more urgent problem: it is not currently racing (nobody holds it), it is silently failing to be re-claimed while a cron gives the appearance of upkeep.
...
|---|---|---|---|
| **Updated 2026-09-25: shipped 2026-09-09, then silently broken 2026-09-13.** The cron line is live (`crontab -l` confirmed today) but every run fails with `not yours; claim first` because the lock was released 2026-09-13 and never re-claimed. Re-claim the seat for whoever holds it under `hierarchy_2026_09_13` (currently `vault`), or point `ZORCA_OWNER` at the real current seat-holder, so the existing cron line starts succeeding instead of quietly no-opping | @Zaal | Config | 2026-09-26 |
| **Updated 2026-09-25: superseded, not simply confirmed.** The primary/secondary/tertiary tiers did go live 2026-09-09 as recommended, but a newer, different hierarchy (`hierarchy_2026_09_13`: vault=top seat/no-code, zorca=first reviewer, zj=build lane) replaced it four days later, confirmed directly by Zaal. Re-run this doc's section 3-4 analysis against `hierarchy_2026_09_13` specifically, since the authority table above still assumes zj=primary | @Zaal | Decision | 2026-10-03 |
| **Updated 2026-09-25: NOT shipped.** `notes/agent-registry-seed-2026-09-09.md` still carries free-prose `role` values on all 63 rows and 28 of 63 (44%) `escalation` cells still read `UNMEASURED`. Edit the seed so every row's `role` is one of `primary`/`secondary`/`tertiary` (against whichever hierarchy wins the action above) and every `escalation` cell names a row, then regenerate `AGENTS.md` | @Zaal | PR (zao-vault) | 2026-10-03 |
| **Updated 2026-09-25: NOT shipped, 3 days overdue.** `git log --oneline -- bin/zorca-lock` shows one commit total, from 2026-08-27 - zero changes since this doc's original write. Port `packages/heart-fleet/`'s acquire/renew/reclaim + fencing-token interface onto `zorca-lock`, scoped to the single "orchestrator seat" resource, with the same 5 safety properties tested (shipped = merged zaal-dotfiles PR, `zorca-lock --selftest` exists and asserts stale-fence rejection) | @Zaal | PR (zaal-dotfiles) | 2026-10-03 |
| **Updated 2026-09-25: NOT shipped**, and the underlying assumption was wrong - `zorca-lock` has never written to `~/.zao/orca-board.log` for any claim, fresh or stale (confirmed: zero matches in 24,499 lines). Add logging to `zorca-lock claim` itself (both the ordinary and stale-takeover paths), not just a stale-takeover branch, then verify by claiming over a manually-staled lock and reading the new log line | @Zaal | PR (zaal-dotfiles) | 2026-10-03 |
...
### Re-verification sources, 2026-09-25

- `~/bin/zorca-lock status` and `cat ~/.zao/orchestrator.lock` - **[FULL - local, executed 2026-09-25 20:47 EDT]** `status` reads `free`; lock file does not exist (`cat: No such file or directory`).
- `~/bin/zorca-lock` (full script) - **[FULL - local file, read in full]** byte-identical to the 2026-09-09 version quoted in section 2: no generation, no fencing, no `--selftest`, no compare-and-swap.
- `git -C ~/zaal-dotfiles log --oneline -- bin/zorca-lock` - **[FULL - local, executed]** exactly one commit, `2837fc6`, dated 2026-08-27 14:40:09 -0400 - zero commits since.
- `crontab -l` on this Mac - **[FULL - local, executed 2026-09-25]** shows `*/15 * * * * ... zorca-lock heartbeat zj >> ~/.zao/zorca-lock-heartbeat.log`, live.
- `tail -30 ~/.zao/zorca-lock-heartbeat.log` - **[FULL - local file, read]** every visible line reads `not yours; claim first`.
- `~/.zao/orchestrators.json` - **[FULL - local file, read in full, 2026-09-25]** `_seat_log` confirms zj released `zorca-lock` 2026-09-13 when the seat moved to vault; a new top-level key `hierarchy_2026_09_13` records a materially different hierarchy (vault=top seat/no-code, zorca=first reviewer, zj=build lane) confirmed directly by Zaal the same day.
- `~/zao-vault/notes/agent-registry-seed-2026-09-09.md` - **[FULL - local file, read/parsed with awk, 2026-09-25]** 63 data rows; `role` column still free prose on every row; `escalation` column reads `UNMEASURED` on 28 of 63 rows (44%).
- `grep -i "zorca-lock" ~/.zao/orca-board.log` (1,826,792 bytes, 24,499 lines) - **[FULL - local file, executed 2026-09-25]** zero matches - `zorca-lock` has never logged to this file.
