...
| # | Decision | Why |
|---|---|---|
| 1 | **DO NOT send `outbound/2026-09-20-reply-to-brandon-conformance.md` as written.** Fix two claims first. | It names `1c6d3b19...` as "your current upstream". That SHA is the **v0.1.0 annotated tag object** (it points to commit `f21aafc`, 2026-07-14), and his upstream HEAD is `611795b` (2026-07-19). The runtime-identity claim still holds against HEAD (`diff -rq` of zol `vendor/dreamloops/runtime/{src,schemas}` against upstream `packages/runtime/{src,schemas}` at HEAD is empty today). The draft also says zol "runs" the manifests, and section 2 shows they run nowhere measurable. |
...
- **Not searched:** iMessage (where this conversation actually lives; no lane can read it), Gmail, Discord, Google Drive (**his stated documentation home**), the Windows PC (the ssh aliases do not resolve; the `baraza` Remote Control channel was not tried), and the "fences"/"authority.v1" code search (cut off by the rate limit, so that one is UNKNOWN rather than absent).
...
| PC reachable **over ssh** (the `baraza`/`pc`/`wsl` aliases) | no | no | HOLDS for ssh only. This does not show that nothing reaches the PC: a Remote Control channel named `baraza` appeared earlier today in the session list, and it was not tried |
...
| Reach the Windows PC over the `baraza` Remote Control channel and check whether any dreamloop manifest runs there. Done when the result, or the fact that it could not be reached, is recorded in dreamnet-zao `measured/` with the method named. Neither draft goes to Brandon before this | dreamnet lane | Measurement | 2026-09-26 |
