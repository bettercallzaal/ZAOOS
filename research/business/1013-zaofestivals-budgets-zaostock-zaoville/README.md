...
- **Permits**: *"$50-$300"*, but only *if* the event is on public/city property. Confirmed 2026-07-11: ZAOville's venue is DCoop's own house (private property) - permit line is $0.
...
Given ZAOville doesn't charge admission, doesn't need a purchased sound system, is co-hosted by the venue owner, and is confirmed on private property (no permit), a realistic first budget is: **Insurance ($300-600) + Permits ($0, confirmed private venue) + Marketing/promotion ($50-200) + Artist/host incidentals - snacks, water, small thank-you gifts ($100-300) + Contingency (~15%, ~$100)**, landing the total between **roughly $450 and $1,100**. [FULL - 3 external open-mic/small-event cost sources plus one municipal rate-card sanity check, applied against the confirmed in-kind equipment donation from the live `/zaoville` page and the confirmed private-venue status]

### 5. RESOLVED - ZAOville's venue is private (DCoop's house), no permit needed

Every open-mic cost source checked treats "is a permit required" as the single biggest binary cost swing (the difference between $0 and $50-300, plus potentially insurance requirements changing). The live `/zaoville` page names "The VEC" as the equipment provider and DCoop as co-host/founder but never stated the venue type. Confirmed directly by Zaal 2026-07-11: ZAOville is hosted at DCoop's house - private property, no permit required. This closes the single biggest open swing factor in ZAOville's budget; the range tightens to $450-$1,100 (Finding 4). [FULL - confirmed directly by Zaal this session]
