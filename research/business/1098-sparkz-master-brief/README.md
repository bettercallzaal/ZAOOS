...
## Locked decisions (2026-07-15) + remaining opens

Resolved in the design session:
- Launch bar: HIGHER / more selective (raise above the prototype's 60; being selective IS the brand - Sparkz wins on coins that survive, not volume).
- First proof-launch: ZAAL dogfoods his own coin through Sparkz first, then recruits a ZAO musician as the first real creator.
- Fotocaster: PENDING - reach the builder to learn its mechanics before it goes in the design. Not in the MVP until known. Tortoise is the confirmed integration.

Remaining open (do not block the MVP):
- The 1% fee BASIS is still being decided (trade volume vs launch raise vs creator-rewards). It is a config value; the MVP builds the mechanics and leaves the basis pluggable.
