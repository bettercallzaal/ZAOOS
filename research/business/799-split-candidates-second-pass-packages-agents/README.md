...
## Drift flags surfaced (need a Zaal decision, not auto-fixed)
1. **`bot/src/teams/` runs Magnetiq + AttaBotty bots** (own tokens, `zao-team-bots.service`, 6 `team_bot_*` tables) — but CLAUDE.md says the branded-bot fleet (naming Magnetiq) was **decommissioned 2026-05-04** and "folds into ZOE memory blocks, NOT new bots." Code contradicts the docs. Don't carry decommissioned bots into a spinout — resolve first.
