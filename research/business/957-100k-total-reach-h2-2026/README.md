...
| 5 | USE Farcaster as the conversion/community layer, not the reach engine - and lean on the mini-app notification loop. | Network is 40-60k DAU total (down from 100k peak); you cannot get 100k reach there. But mini apps are the documented growth loop (breakouts grew "almost entirely through frame embeds"), notification tokens re-engage (batch 100/request, Neynar analytics), and Season Run + the submission platform are already built for exactly this. |
