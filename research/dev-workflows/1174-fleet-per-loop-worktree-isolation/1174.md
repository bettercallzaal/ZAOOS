...
Reviewed per batch task. **ZOL is already properly isolated — confirms current setup is correct.**

ZOL's working directory (`/home/zaal/zol-upgrade`) is its own dedicated clone (not a worktree of zao-os). The collision described (zoe + human both on ~/zao-os simultaneously) cannot happen to the ZOL loop. ZOL satisfies Rule 25 by design.

The `loops-keepalive.sh` change (pointing zoe/human to separate worktrees zao-os-zoe/zao-os-human) is a Zaal action on the Pi. The proposed `git -C "$ZAO_MAIN" worktree prune` on startup is also good hygiene.

For ZOL's own sub-agents: the ZOL loop's `isolation: 'worktree'` pattern in Agent tool calls already creates isolated worktrees per subagent when needed. Task leasing (IdempotencyStore, board.task.claim conditional PATCH) from PR #35 handles the write-dedup side.

No ZOL code changes. Blocked: Zaal to apply the diff to `~/bin/loops-keepalive.sh` on the Pi.
