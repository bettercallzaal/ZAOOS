...
## Updated 2026-09-25 - re-research pass

Central claim of the audit (18 LOUDER / 6 QUIETER / 3 CANNOT TELL, with two
design rules) is **not overturned** - the class is real and the two design
rules still hold. But three of the six QUIETER findings this doc fed directly
into a PR, and the PR shipped more than the doc asked for. Re-verified today
against live state (VPS over ssh, ZAOOS repo on disk, `git log`), not against
the 2026-09-08 text:

- **FIXED, PR #3458 (2026-09-12), three items in one commit:** `GET
  /api/bots/status` now returns 502 + an explicit `error` field instead of a
  silent `200` + `bots: []` when the upstream shape is malformed (with a test
  that pins the fix and a control test proving a genuinely empty fleet still
  reads 200 clean). `estate-health.yml`'s ratchet gained the `crashed`
  severity this doc's Key Decision 4 asked for - `runOne`/`scoreAndSummarize`
  are now exported and covered by three tests (a thrown check scores
  `crashed`, counts toward `summary.fail`, and a clean check still scores
  100). `research-index.yml`'s backfill-commit step no longer degrades a push
  failure to a warning - **and a follow-up review caught that a bare
  `git push || exit 1` would have failed ~10% of triggering merges on a lost
  race rather than a real fault** (measured on 266 triggering commits over 30
  days: 26 pairs within 120s, 54 within 300s), so the shipped version fetches,
  rebases onto `origin/main`, retries once, and only then hard-fails. This is
  a better fix than the one this doc's Next Actions table asked for, not just
  a different one - flagged because a QUIETER-to-LOUDER fix that overcorrects
  into "reds main on every race" is the same failure mode with the sign
  flipped, and this repo caught that on itself.
- **FIXED, PR #3456 (2026-09-08, same day as the original measurement, timed
  after it):** `zao-wall.py --release`'s wrong-destination print is gone -
  `report()` now derives its message from `where(card)`, the same function the
  views use, rather than printing a hardcoded string keyed to the verb. The
  specific card this doc named, `ef98e806`, is **no longer stuck under
  `route=prep`** - a separate fix the same week (`0f22014bb`, flag-based
  routing) moved it to the `flagged` (needs-Zaal) bucket instead, because it
  carries `metadata.decision`. Re-read live today via `zao-wall.py --json`:
  `open=390` (down from 581 on 2026-09-08), but **`unrouted=263`, all
  `route=none`** - the general unrouted bucket has grown roughly 20x since
  the last measurement, not shrunk, even though the specific bug and the
  specific card are both resolved. The claim "the backlog this class of bug
  fed has been worked down" no longer holds as stated; worth its own
  follow-up, out of scope for this doc's specific instance.
- **NOT shipped, still the live gap:** `fleet-spend-guard.sh.proposed` (the
  alert-dedup flag from Key Decision 2 / Next Action 1) still sits next to
  the live script on the VPS, unchanged, mtime `2026-09-08 11:01`, never
  installed. **This matters more today than it did on 2026-09-08**: the
  OpenRouter account is overdrawn again - `total_credits=131,
  total_usage=131.095996071`, balance **-$0.096** (re-measured today via
  `curl .../v1/credits`, not assumed) - below the `$5` floor, and the
  crontab (`*/20 * * * * fleet-spend-guard.sh`) runs the live, non-deduped
  script every 20 minutes. The live script has no `BALFLAG`-equivalent state
  file (confirmed: `~/.zao/openrouter-balance.alerted` does not exist on the
  VPS), so it is sending a fresh low-balance Telegram message every 20
  minutes right now - the exact repeat-alert problem the proposed fix exists
  to solve, live, not hypothetical.
- **NOT shipped:** `fleet-health.sh` and `cost-of-pass-summary.sh --tg` still
  send via bare `curl -s ... >/dev/null` with no response-body check (grepped
  today; unchanged from 2026-09-08). `healthchecks.io` (or an equivalent
  dead-man's switch) has not been adopted anywhere - `grep -rl
  "healthchecks.io\|hc-ping" ~/bin/` on the VPS and `crontab -l | grep -i
  healthchecks` both return no matches today.
- **Not re-audited this pass:** the remaining ~21 of the 27 checks in the big
  audit table below (local mac, most of the VPS crontab, the GitHub Actions
  not touched by PR #3458, the ZAOstock workflows) were not re-measured today.
  Treat those verdicts as last confirmed 2026-09-08, not as re-verified
  2026-09-25 - marked UNKNOWN-since-last-check rather than silently carried
  forward as current.
...
**2026-09-08:** already fixed and live (`~/bin/fleet-spend-guard.sh` on the VPS, mtime 2026-09-08 10:38, well ahead of when this doc started). It checks both the RATE (`usage_daily` vs `$5/day` cap) and the TANK (`total_credits - total_usage` vs a `$5` floor). A failed read on either metric sets the guard's own reported state to `UNKNOWN`, sends a Telegram alert saying so explicitly, and **never lifts the pause flag on a failed read** - defect 2 above is closed. Balance that day: `total_credits=100`, `total_usage=90.131049664`, **balance `$9.869`** - recovered well above the `$5` floor. A second version, `fleet-spend-guard.sh.proposed` (mtime 11:01, newer than the live file), adds an alert-dedup flag so the low-balance message fires once per crossing instead of every 20-minute tick; it was not yet installed.

**Re-verified 2026-09-25: the RATE/TANK fix is still live and unchanged (`diff` against the file read on 2026-09-08 shows no code drift beyond the still-unmerged `.proposed` file). The dedup fix still is not installed** - `fleet-spend-guard.sh.proposed` sits next to the live script, same mtime (`2026-09-08 11:01`), never merged in. **And the account is overdrawn again**: re-measured today via `curl https://openrouter.ai/api/v1/credits` - `total_credits=131`, `total_usage=131.095996071`, balance **-$0.096**, below the `$5` floor. `~/.zao/openrouter-balance.alerted` does not exist on the VPS, so the live (non-deduped) script is sending a fresh low-balance Telegram message on every 20-minute cron tick (`*/20 * * * * fleet-spend-guard.sh`, confirmed in `crontab -l`) for as long as the balance stays under floor - the exact repeat-alert failure mode Key Decision 2 named, now live rather than hypothetical. The RATE/TANK correctness fix held for 17 days without regressing; the availability/noise gap named on day one is still open on day 17.
...
### 3. `zao-wall.py --release` reporting the wrong destination - FIXED same day (was STILL BROKEN at first measurement, closed before this doc's own last-validated date)

`--release ef98e806` printed `"released to unclaimed: ef98e806"`; the card actually landed in `UNROUTED/prep`, a bucket no view showed at the time. At the moment this doc's Instance 3 text was first drafted, `set_lane()` (`scripts/agents/zao-wall.py` line 137-138) only popped `metadata.lane` and never touched `metadata.route`, and the print statement at line 212 was unconditional.

**Fixed the same day, PR `d959c7a1` (2026-09-08 11:57 EDT, "wall: a human destination (--to-zaal) and every write reports where the card actually landed", #3456).** `report()` now calls the same `where(card)` function the views use and prints the label it returns - "The message comes from the card, never from the verb" is now the function's own docstring. Read directly today (`scripts/agents/zao-wall.py` lines 250-262).

**Re-verified 2026-09-25 via `zao-wall.py --json` against the live board:** `ef98e806-b6f9-4388-ad50-7de76aa7da8b` is no longer in `unrouted` at all - a separate fix the same week (`0f22014bb`, flag-based routing ahead of lane/route) moved it into the `flagged` (needs-Zaal) bucket, because it carries `metadata.decision`. Today's live counts: `open=390, on_wall=15, unclaimed=47, needs_zaal=65, flagged=12, unrouted=263` (`none: 263, prep: 0`). **The specific card and the specific `--release` bug are both closed. The general unrouted bucket has grown roughly 20x since 2026-09-08 (12 -> 263), not shrunk** - the "backlog worked down" framing from the first draft of this doc does not hold today; this is a live, unaddressed finding of its own, out of scope to fix here.
...
For every check, monitor, guard and health endpoint reached in this pass: **if the thing it watches failed right now, would this get LOUDER or QUIETER?** 27 checks audited across five surfaces (mac, VPS, ZAOOS API, ZAOOS GitHub Actions, ZAOstock GitHub Actions) as of 2026-09-08. Totals then: **18 LOUDER, 6 QUIETER, 3 CANNOT TELL.**

**Updated 2026-09-25:** three of the six QUIETER rows (`GET /api/bots/status`, `estate-health.yml`'s PR ratchet, `research-index.yml`'s backfill push) are now confirmed FIXED and reclassified LOUDER below, via PR #3458 (2026-09-12). Re-measured totals for the rows actually re-checked this pass: **21 LOUDER, 3 QUIETER, 3 CANNOT TELL** (`fleet-health.sh`, `provider-health.sh`, `cost-of-pass-summary.sh` remain QUIETER, confirmed unchanged today). The other 21 of 27 rows were not re-audited this pass - their verdicts below are carried forward from 2026-09-08 and should be read as last-confirmed then, not re-verified now.
...
| `GET /api/bots/status` | **FIXED, now LOUDER (was QUIETER)** | Proxies the cowork board's `/api/v1/bots`. A non-2xx response correctly returns `502` with an `error` field (LOUDER). **Fixed 2026-09-12, PR #3458**: a malformed upstream body (`data.bots` not an array) now returns `502` + an explicit `error` field instead of silently falling back to `bots: []` with a `200`. A control test (read today, `src/app/api/bots/status/__tests__/route.test.ts`) pins that a genuinely empty fleet still returns `200` with no error, so the fix cannot regress back to the inverted shape without a red test. |
...
| `estate-health.yml` - PR guardrail ratchet | **FIXED, now LOUDER (was QUIETER)** | See Key Decision 4. Was: `runOne()` converted any individual check's exception into `{status:'skipped', findings:[]}`, so a crashed check contributed zero to `summary.fail` - identical, numerically, to a clean pass. **Fixed 2026-09-12, PR #3458**: a `crashed` severity now scores above `fail` and counts toward `summary.fail`, so the ratchet trips; `runOne`/`scoreAndSummarize` are exported and covered by three tests (read today, `tools/estate-control-plane/__tests__/checks.test.ts`) proving a thrown check scores `crashed`, that it moves the ratchet, and that a clean check still scores 100. Supersedes the `estate-health.yml` finding in [`security/2093-silent-failure-sweep`](../../security/2093-silent-failure-sweep/) (finding 6), which described an earlier version of this workflow before the rewrite; doubly superseded now that this doc's own finding is also closed. |
| `research-index.yml` (CI-owned backfill) | **FIXED, now LOUDER (was QUIETER)** | Was: the commit-back step ended `git push \|\| echo "::warning::..."` and still exited `0` on a push failure - a stopped backfill and a complete index rendered identically green. **Fixed 2026-09-12, PR #3458, then hardened further the same day on review**: a bare hard-fail would have reddened `main` on a lost race rather than a real fault - measured on 266 triggering commits over 30 days, 26 pairs landed within 120s and 54 within 300s of each other, roughly a tenth of triggering merges. The shipped version fetches, rebases onto `origin/main`, retries the push once, and only hard-fails (`exit 1`) if that retry also fails - branch protection or an expired token still fail loud; a lost race no longer does. Read today, `.github/workflows/research-index.yml` lines 31-75. |
...
**Updated 2026-09-25 - shipped since 2026-09-08:** the `zao-wall.py --release` fix (same day, PR #3456), the `estate-health.yml` crashed-severity fix (PR #3458, 2026-09-12), the `research-index.yml` hard-fail fix (same PR, then hardened further the same day per zaoos-review), and the `GET /api/bots/status` fix (same PR) - none of these four were in the original Next Actions table by name (the table asked only for the run-checks.ts and research-index.yml items; the `--release` and `bots/status` fixes shipped as a bonus in the same effort). Two of the five original rows below are still open, re-confirmed today, dates refreshed.

| Action | Owner | Type | By When |
|---|---|---|---|
| Install `fleet-spend-guard.sh.proposed`'s alert-dedup flag over the current live VPS script - shipped when the low-balance Telegram message fires once per crossing instead of every 20 minutes. **Still open 2026-09-25**: `.proposed` file unchanged since 2026-09-08 11:01, never merged; the account is overdrawn again (-$0.096 today) and the live script is alerting every 20-minute cron tick with no dedup, right now | Zaal | Config (VPS `~/bin`) | 2026-09-27 |
| ~~Fix `zao-wall.py --release`...~~ **SHIPPED 2026-09-08, PR #3456 (`d959c7a1`)** - `report()` now derives its message from `where(card)`. Confirmed live today. | Zaal | Done | 2026-09-08 |
| ~~Add a `crashed` severity to `run-checks.ts`...~~ **SHIPPED 2026-09-12, PR #3458** - `crashed` scores above `fail`, three tests confirm it moves the ratchet. Confirmed live today. | Zaal | Done | 2026-09-12 |
| ~~Change `research-index.yml`'s backfill-commit step...~~ **SHIPPED 2026-09-12, PR #3458, then hardened same day** - fetch/rebase/retry-once before hard-failing, closing a false-positive race the plain hard-fail would have introduced (measured ~10% of triggering merges land within 300s of each other). Confirmed live today. | Zaal | Done | 2026-09-12 |
| Point `fleet-health.sh` and `cost-of-pass-summary.sh --tg` at the same "check the Telegram response body" pattern `loops-report.sh` already uses - shipped when both scripts' code contains the `"ok":true` check instead of a bare `curl -s ... >/dev/null`. **Still open 2026-09-25**, re-confirmed unchanged via ssh grep | Zaal | Config (VPS `~/bin`) | 2026-10-03 |
| Adopt `healthchecks.io` (or equivalent) as a dead-man's switch on the 9 mac crons and the active VPS crontab lines that pipe to `/dev/null`, per doc 2450's recommendation, starting with `stall-tripwire.py` and `cost-of-pass-summary.sh` - shipped when deliberately killing one of those two produces an email within its expected window. **Still open 2026-09-25**, re-confirmed via ssh grep (`healthchecks.io`/`hc-ping` absent from `~/bin` and crontab) | Zaal | Config | 2026-10-03 |
| NEW 2026-09-25: fix the general `unrouted` bucket growth (12 -> 263 open cards since 2026-09-08, all `route=none`) surfaced as a side effect of re-verifying instance 3 - separate from the `--release` bug, which is closed | Zaal | Investigate (ZAOOS) | 2026-10-03 |
...
- [`ZAODEVZ/ZAOstock` - `.github/workflows/uptime.yml`](https://github.com/ZAODEVZ/ZAOstock/blob/main/.github/workflows/uptime.yml) and [`ci.yml`](https://github.com/ZAODEVZ/ZAOstock/blob/main/.github/workflows/ci.yml) - [FULL, method: `gh api repos/.../contents/... --jq .content | base64 -d`, 2026-09-08]. `gh api repos/bettercallzaal/{wwbase,wavewarzapp,wavewarz-overlay}/contents/.github/workflows` (all 404, confirmed absent) and `gh api repos/ZAODEVZ/ZAOstock/contents/src/app/api` (listed, no health/status route) - [FULL, executed 2026-09-08]. Not re-checked 2026-09-25; treat as last-confirmed 2026-09-08.

### Re-research pass, 2026-09-25 (re-fetched, not trusted from the 2026-09-08 text)

- `ssh zaal@31.97.148.88` - `~/bin/fleet-spend-guard.sh`, `.proposed`, `crontab -l`, `~/.zao/cheap-loop.pause`, `~/.zao/openrouter-balance.alerted`, `~/bin/fleet-health.sh`, `~/bin/cost-of-pass-summary.sh`, `~/bin/loops-report.sh` - [FULL, read/diffed over ssh, 2026-09-25].
- `curl https://openrouter.ai/api/v1/credits` from the VPS with the live key, 2026-09-25 - [FULL] - `total_credits=131, total_usage=131.095996071, balance=-$0.096`.
- `grep -rl "healthchecks.io\|hc-ping" ~/bin/` and `crontab -l | grep -i healthchecks` on the VPS - [FULL, executed, zero matches - a real negative result, not a tool failure; confirmed the grep pattern matches on a control string first].
- `/Users/zaalpanthaki/Documents/ZAO OS V1/scripts/agents/zao-wall.py` lines 100-262, read on disk 2026-09-25 - [FULL].
- `python3 scripts/agents/zao-wall.py --json` against the live cowork board Supabase, 2026-09-25 - [FULL] - `open=390, on_wall=15, unclaimed=47, needs_zaal=65, flagged=12, unrouted=263 (none: 263)`; `ef98e806` located in `flagged`.
- `git log`/`git show` in `/Users/zaalpanthaki/Documents/ZAO OS V1` for `d959c7a1` (#3456), `5c2719ff1`/`d948a37dc` (#3458), and `0f22014bb` (#3452) - [FULL, read on disk, 2026-09-25] - full commit messages read, not just subject lines.
- `/Users/zaalpanthaki/Documents/ZAO OS V1/tools/estate-control-plane/run-checks.ts` and `__tests__/checks.test.ts` - [FULL, read on disk, 2026-09-25].
- `/Users/zaalpanthaki/Documents/ZAO OS V1/.github/workflows/research-index.yml` - [FULL, read on disk, 2026-09-25].
- `/Users/zaalpanthaki/Documents/ZAO OS V1/src/app/api/bots/status/route.ts` and `__tests__/route.test.ts` - [FULL, read on disk, 2026-09-25].
