...
| # | Decision | Why |
|---|---|---|
| 1 | RUN one Orca lane on OpenCode against a non-Anthropic provider (OpenRouter, GLM/Z.AI, or a local Ollama model) for low-stakes lanes (doc grunt work, triage, research fetches) - this is the only configuration of either tool that actually removes load from the Claude weekly cap. | OpenCode has no built-in Claude subscription reuse - the Anthropic entry in its 75+-provider list (`opencode.ai/docs/providers`) is API-key only (`ANTHROPIC_API_KEY`), so any Claude-model use there is billed separately from the Max/Pro plan; pointed at a different provider it touches Claude usage not at all. |
...
- [OpenCode docs: Providers](https://opencode.ai/docs/providers) - [FULL, method: curl + HTML strip] fetched 2026-09-13, 424KB raw HTML / ~54K chars stripped text.
