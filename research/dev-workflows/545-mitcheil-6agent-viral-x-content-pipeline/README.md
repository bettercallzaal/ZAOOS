...
|----------|----------------|-----|
| **Replicate the 6-agent structure** | USE — partial. Build a `bcz-content-pipeline` agent system on VPS 1 with 4 of 6 agents (Hook, Script, Critic, Output). Skip "internet scan via paid X API" + "5,000 post pull" until ROI proven. | Already have RaidSharks + Empire Builder for amplification; pipeline upstream is the gap. |
...
| **5,000 post research sweep via paid X API** | SKIP for now. Use `exa` MCP semantic search instead. Free + already integrated. | X API basic tier costs $200/mo and rate limits hard. Exa covers cross-posts on Substack/Medium. Re-evaluate if content volume scales 10x. |
...
| **Hard numeric gates (4 hooks × 3 iterations × 5D scoring)** | USE — bake into pipeline as eval criteria. | Aligns with `eval-harness` + `gan-style-harness` skills. |
| **Copy-paste-ready 4 hooks + 2 CTAs at end** | USE — every BCZ content deliverable ships with this packaging. | Removes Zaal's manual "what do I post?" step. |
| **Content topics queue** | USE — start with: Hermes shipping cadence, Quad multi-agent dev fleet, ZAOstock launch, Cipher release, ZAO 101 Concierge demos | Highest-leverage upcoming announcements. |
...
| Empire Builder V3 (in-progress, see `project_raidsharks_empire_builder.md`) | downstream amplification |
...
| Hermes (just shipped sprint 1 cost routing, doc 541) | could host the pipeline |
...
Each is a content series, not a single post.
...
| Pick 1 topic from the 7 above, run pipeline E2E as a pilot | Zaal | Decision | This week |
...
- **Banned content rules apply:** no em dashes, no emojis, exact brand spelling per global CLAUDE.md glossary.
...
- [Doc 506 — TRAE AI SOLO skip](../506-trae-ai-solo-bytedance-coding-agent/) — Plan Mode + Sub Agent + DiffView pattern theft
- [Doc 536 — darkzodchi CLAUDE.md best practices](../536-darkzodchi-claude-md-best-practices/) — companion CLAUDE.md design from same week
- [Doc 537 — Shann's Superpowers planning framework](../537-shann-superpowers-planning-framework/) — companion planning framework
...
3. [Anthropic Claude Code sub-agents docs](https://docs.claude.com/en/docs/claude-code/sub-agents) — sub-agent reference
