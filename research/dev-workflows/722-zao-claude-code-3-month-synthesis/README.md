...
| [722i Bonfire KG state](722i-bonfire-kg-state/) | What is in the ZABAL Bonfire graph, who writes to it, what is queryable | 3 active writers: ZOE `recall.ts` mirrorTurn, `/bonfire` skill, legacy bulk-ingest scripts. 1100+ nodes. 120 new episodes this session. Read-locked - `/vector_store/search` returns `[]` until Joshua.eth runs admin labeling. Memory backfill (174 files) is the unblock dependency for doc 620. |
| [722j Public-surface map](722j-public-surface-map/) | Every public domain, social handle, Telegram bot/group, on-chain surface in the ZAO footprint | Front door = zaoos.com/zabal (doc 708 hub). 5 Telegram bots live, Farcaster /zao channel (188 members), all major domains live (zaoos.com, bettercallzaal.com, bczyapz.com, cocconcertz.com, wavewarz.com). Dead: thezao.com, fishbowlz.xyz. ~30 URLs in nexus.html need quarterly health-check. $ZABAL token + Empire V3 + app FID 19640 on-chain. |
| [722k Content + social cadence](722k-content-social-cadence/) | What is actually shipping content-wise vs what is planned but cold | LIVE: ZOE 7 pings/day (PR #533), WaveWarZ 11 shows/wk, BCZ YapZ 8 episodes since graduation, Sunday 5/24 Farcaster spaces locked. COLD: newsletter (code ready, zero adoption), `/big-win` (model exists, not integrated), 12-week Road to ZAOstock (July launch). Block: Firefly API missing -> 7-step manual cross-post per content. |
