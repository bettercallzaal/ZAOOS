...
### 6. RESOLVED 2026-07-13 - a real crypto-payment path exists, and ZAOville (Jul 25) is the actual deadline, not Oct 3

Zaal flagged that cash is too tight (~$1.5K on hand across both events) to spend $575-625 on this gear unless it can be paid in crypto, and that ZAOville (Laurel, MD - **July 25, 2026**, ~12 days out) needs a working livestream setup before ZAOstock does. Researched both questions directly:

- **No major AV retailer (B&H, Adorama, Amazon, Blackmagic's own store, Sweetwater) accepts crypto directly at checkout.** That path doesn't exist.
- **A crypto debit card (Coinbase Card or BitPay Card) does work**, and is the cleanest real path: spending USDC held in a wallet through the card is treated as a normal Visa purchase at Amazon or B&H Photo, with 0% conversion fee on stablecoin spend (no gift-card hop, no separate merchant integration needed) and instant checkout. Gift-card workarounds (Bitrefill, ~1-2% fee, Amazon-only) and virtual-card services (Moon, B&H/Adorama-compatible) are viable fallbacks if a Coinbase/BitPay card isn't already set up, but add friction.
- **Recommended sequencing given the Jul 25 deadline**: rent an ATEM Mini Pro for the ZAOville date ($35-58/day from AV rental houses) rather than buy outright under time pressure, paid via crypto debit card if the rental company takes card payment (confirm directly - not all rental houses do). In parallel, buy a cheap USB audio interface (~$30-50, e.g. Behringer UMC202HD) now as a same-week Amazon order (crypto-card-payable), which also works as the permanent audio-chain piece doc 1030's Finding 4 already specced. Buy the ATEM Mini Pro outright for ZAOstock once there's real lead time (August, not this week) - full $325 crypto-card-payable with no fee.
...
| ~~Buy the ATEM Mini Pro ($325) and the audio DI/interface bundle ($250-300)~~ - RESEQUENCED 2026-07-13: rent a switcher for Jul 25 ZAOville (~$35-58, crypto-card-payable if the rental house takes card) + buy a cheap $30-50 USB audio interface now, defer the full $325 ATEM purchase to August for ZAOstock | Ohnahji/Zaal | Task | 2026-07-20 (rental+interface), 2026-08-15 (ATEM purchase) |
