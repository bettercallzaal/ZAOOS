---
topic: events
type: decision
status: research-complete
last-validated: 2026-07-11
superseded-by:
related-docs: "1007, 1013, 1030, 1031, 1032, 1033, 1034"
original-query: "find more ways to optimize (ZAOstock, after 9 research docs shipped in one session)"
tier: STANDARD
---

# 1035 — ZAOstock Master Punch List: Every Open Action, Deduplicated and Prioritized

> **Goal:** Nine ZAOstock research docs shipped in this session (1007, 1013, 1030-1034), each with its own Next Actions table. Several actions appear in two different docs under different framing. Nobody can execute against nine scattered tables. This doc is the single, deduplicated, priority-ordered list - the connective tissue the research itself was missing.

## Key Decision

The real optimization at this point in the project isn't more research - it's stopping research sprawl and making everything already found executable. This doc does that: 27 distinct actions across 9 docs collapse into 19 real items once duplicates are merged, sorted into four urgency tiers instead of nine separate By-When columns.

**Update 2026-07-11:** two more Tier 1 items are RESOLVED and removed below. (1) ZAOville venue confirmed private (DCoop's house), no permit needed - see doc 1013 for the updated budget ($450-$1,100). (2) The "$391K in grants" HoE figure is confirmed accurate against HoE's own 2025 Annual Report - see doc 1031. Also shipped this session: the new `/acadia` page and the `/ellsworth` additions (doc 1034) are live.

**Update 2026-07-12:** Zaal answered a batch of Tier 1/2 questions directly. Site Lead + First Aid Lead: both Zaal (see the caveat under that row - worth a second look, not a clean resolution). Cadillac Mountain reservation: confirmed NOT yet booked. Sound-system booking: confirmed NOT yet booked. ZAOville's pool-coverage plan and pre-event content owner are resolved in doc 1036. Livestream/broadcast lead: re-asked after a ring-buffer eviction lost the first answer - now confirmed **Ohnahji** (see doc 1030 Finding 5). Every Tier 1 item is now either resolved or confirmed-still-open, none are silently stale.

**Correction 2026-07-12:** doc 1013's $20K/$25K/$7K budget figures were WRONG - audit-inflated, per Zaal's direct correction. Real ZAOstock target: ~$5K at most, ~$1.5K on hand. Doc 1013, doc 1030, and this doc's Tier 2 gear-purchase item have all been corrected. The $325 ATEM Mini Pro + $250-300 audio bundle (Tier 2 below) now needs to be weighed against a real ~$5K budget with ~$1.5K on hand - not the comfortable $20K margin this doc originally assumed.

**Update 2026-07-13:** Zaal reprioritized several items directly. (1) Cadillac reservation: Zaal questioned whether a reservation/pass is even needed after early October - **re-verified directly against NPS/recreation.gov, and it turns out the earlier finding was right**: Oct 3 falls inside the required-reservation window (through Oct 25) and the park entrance fee applies year-round regardless. Still a real, still-open Tier 1 item - see the corrected row below with exact costs. (2) Sound system: decision is deliberately paused until Thursday 2026-07-16, when the team will see what the Town of Ellsworth itself provides - not a silent gap, a scheduled check-in. (3) The Site Lead/First Aid Lead dual-role concern is real but not urgent - moved to a ~1-month-out item (see Tier 3), not this week. (4) The livestream gear purchase (ATEM Mini Pro + audio bundle, Tier 2) just got more urgent, not less: ZAOville (Laurel MD, **July 25, 2026**) needs this gear before ZAOstock does, and cash is tight enough that the team wants to pay in crypto if at all possible rather than spend from the ~$1.5K on hand - research on crypto-payment/rental/cheaper-alternative paths is in flight. (5) Locking in the pitch decks - both the Artizen fund submission deck and the local sponsor partner deck, built off shared content - is now explicitly the top priority.

## Tier 1 - This week (by Jul 18) - blocking or time-sensitive

| Action | Source doc(s) | Why it's Tier 1 |
|---|---|---|
| **Book the Cadillac Mountain vehicle reservation for Oct 3 - VERIFIED 2026-07-13, still required, do not skip it** | 1034 | Re-checked directly against NPS/recreation.gov after Zaal questioned whether it's needed: the reservation-required window for 2026 is **May 20 - October 25**, so Oct 3 falls squarely inside it - a reservation ($6/vehicle) IS required, not optional. Separately, Acadia's park entrance fee ($35/vehicle, 7-day pass) is required year-round regardless of the summit-road reservation - it does NOT lapse in October. Total: **$41/vehicle minimum** for anyone driving into the park that day. Sources: nps.gov/acad/planyourvisit/vehicle_reservations.htm, nps.gov/acad/planyourvisit/fees.htm, recreation.gov/timed-entry/400000 |
| ~~Name a livestream/broadcast lead~~ - ANSWERED 2026-07-12: Ohnahji | 1030, 1031 (duplicate, merged) | Unowned since May across 3 different docs (609 said Thy Revolution, 720 said Onaji). Doc 720's call is the one that stuck. Ohnahji still needs to actually be looped in on doc 1030's gear stack and doc 1036's ZAOville dry run |
| ~~Name a Site Lead and a First Aid Lead for Oct 3~~ - ANSWERED 2026-07-12: both Zaal | 1032 | Worth a second look, not a clean resolution: doc 1032 explicitly called for a First Aid Lead with current certification, not "whoever's around," and both roles are described as running full-day in parallel during the 12pm-6pm show. One person holding both needs either a real plan for how that works simultaneously, or a second name for one of the two roles |
| ~~Update the stale $5K/$25K ZAOstock budget figure in `project_zao_stock_confirmed.md`~~ - DONE 2026-07-12, but with the correct figure (~$5K/~$1.5K on hand), not doc 1013's retracted $7K/$20K/$25K conclusion | 1013 | The file most likely to get quoted by a future session if not fixed - cheap to fix, keeps compounding if not |

## Tier 2 - Next 2 weeks (by Aug 1)

| Action | Source doc(s) | Why it matters |
|---|---|---|
| **RESEARCHED 2026-07-13, still needs a call: the exemption almost certainly does NOT apply - plan for the standalone permit path** | 1031, 1032 (duplicate, merged), see [Doc 1070](../1070-zaostock-permit-exemption-ems-coverage/) | Ordinance Chapter 14's exemption requires events "sponsored and under the direct supervision of the City" - Art of Ellsworth is run by Heart of Ellsworth (a nonprofit), not the City directly. Real contacts to confirm this and start the surety bond process: City Clerk Suzanne McLean (207-667-2563) or Police Chief Troy Bires (207-667-2168) |
| **Decide sound system after the Thursday 2026-07-16 check-in with the Town of Ellsworth** | 1007 | Still not booked, still 6+ weeks past its own internal "book by June 1" deadline - but Zaal deliberately wants to wait and see what the town itself provides before booking anything separately, rather than duplicate spend. Revisit right after Thursday, don't push further until then. **Backup vendors lined up 2026-07-14 regardless of Thursday's outcome:** Maine Audio Visual (Turner, 207-740-3345, ~1h15 away, statewide service confirmed for Hancock County, documented outdoor community-event experience) is the best-balanced option; DRD Audio/Visual (Fort Fairfield, 207-472-3900, ~2.5h away) has the strongest actual festival track record (35 years, real Maine county fairs) if the extra distance is worth it |
| **Get a current-owner confirmation for lineup (Dcoop), venue+ops, and volunteers workstreams** | 1007 | Only the sponsors workstream (Jay/Duh) is confirmed active; the rest haven't been checked since doc 871 (June 17) |
| **URGENT (real deadline is Jul 25, ZAOville, not Oct 3): get livestream gear sorted - ATEM Mini Pro ($325) + audio DI/interface bundle (~$250-300), crypto payment preferred over cash** | 1030 | ZAOville needs this before ZAOstock does - the actual deadline is 2 weeks out, not mid-August. Cash on hand is ~$1.5K total, so the team wants to pay in crypto if there's a real path to it, or find a cheaper/rental alternative rather than spend ~$600 cash. Research on crypto-payment paths, rental options, and cheaper software-based alternatives (e.g. OBS + a basic USB interface instead of the ATEM) is in flight |
| **Lock in the pitch decks (Artizen fund deck + local sponsor partner deck) - now the explicit top priority** | new, per Zaal 2026-07-13 | Two decks, shared content backbone (proof numbers, story, credibility): the Artizen.fund submission deck (drives the live, time-limited Daybreak Fund Drive) and the local sponsor partner deck (zaostock.com/sponsor/deck + a PDF version for HoE's bank-partner outreach below) |
| **Reach out to one of HoE's 4 Downtown Grants Program bank partners for a warm co-sponsor conversation** | 1031 | Concrete, named list (Franklin Savings, Bangor Federal Credit Union, First National Bank, Machias Savings) via the Zaal builder-profile onepager already drafted - the partner deck above is what actually gets sent in this conversation |
| ~~Build the new `/acadia` page and add the 2 restaurant/shuttle/sculpture-trail items to the live `/ellsworth` page~~ - SHIPPED 2026-07-11 | 1034 | Both pages are live and linked from the homepage footer |
| **RESEARCHED 2026-07-13: EMS standby is a Fire Chief discretionary call inside the same permit review, not automatic - real cost if required is $1,500-2,000+** | 1032, see [Doc 1070](../1070-zaostock-permit-exemption-ems-coverage/) | Same phone call as the permit exemption question above can resolve this - ask the Fire Chief (via 207-667-8666) directly whether standby is required for a 200-400 person event |
| ~~Update `/sponsor` page with the Route 1/Route 3 traffic numbers and Main Street America framing~~ - SHIPPED 2026-07-11 | 1031 | The $20K budget target line added at the same time was REMOVED 2026-07-12 per Zaal's correction - dollar figures now stay off public content entirely |

## Tier 3 - By mid-August

| Action | Source doc(s) | Why it matters |
|---|---|---|
| **Decide whether the Aug 15 dry run (doc 504) is happening, still planned, or dropped** | 1032 | Doc 1032's entire operations plan is untested if the dry run never runs - and Aug 15 is now less than 5 weeks out |
| **Pull a fresh Supabase snapshot of the ZAO STOCK project's team/circle/sponsor/budget data** | 1007 | The last real snapshot is from doc 610, May 5 - everything since is inference from tracker/memory, not a direct check |
| ~~Confirm porta-potty, radio rental (if budgeted), and first-aid supply sourcing~~ - RESEARCHED 2026-07-13, real vendors + prices found, just needs booking calls | 1032, see [Doc 1071](../1071-zaostock-vendor-sourcing-sanitation-radio-firstaid/) | Real local vendor found (Ray Plumbing, Ellsworth, $140-235/weekend for porta-potties); radios and first aid combined run under $150. Total realistic cost $176-730 - the cheapest of the outstanding line items, just needs the actual booking calls made |
| **Do a deliberate photo scouting pass at the Tier-1 Acadia locations** (Cadillac, Jordan Pond, Otter Cliffs, Bass Harbor, Schoodic) | 1033 | Needed to actually seed the Week 3+ slots in the photo/promo calendar |
| **Confirm what the 2 existing artist videos are** (format, length) and whether Fellenz + Dcoop are the only 2 confirmed artists so far - CHECKED 2026-07-13, the site itself has no `video_url` field on artists at all, so this needs Zaal's direct input, not further site digging | 1033 | Determines how the artist content track in the photo/promo calendar is actually built. Real gap surfaced: even once the videos are named, there's currently no way to feature them on an artist profile page without adding a field/UI first |
| ~~Run Supabase's free Database Advisor against ZAOstock's own project for an RLS check~~ - RUN 2026-07-13 against the one reachable project (ZAOcowork tracker, not ZAOstock's own DB) | (carried over from doc 1009/1011's ecosystem-wide finding, applied to ZAOstock specifically) | Confirmed the same credential-exposure pattern is still live - see [Doc 1060](../../infrastructure/1060-zaostock-cowork-tracker-rls-audit/). 12 tables including budget_entries/sponsors/team_members grant full read/write to any authenticated user. Flagged directly to Zaal, awaiting his call on fix priority. Still open: get a token for ZAOstock's actual canonical project so this check can run against the real thing |
| **Get a backup name for First Aid Lead (or Site Lead) so Zaal isn't running both simultaneously on Oct 3** | 1032 | Not urgent - explicitly deprioritized by Zaal 2026-07-13 to a ~1-month-out item, not this week. Doc 1032 called for a certified First Aid Lead specifically, and both roles run full-day in parallel during the 12pm-6pm show - one person can't really do both live |

## Tier 4 - By September / pre-event

| Action | Source doc(s) | Why it matters |
|---|---|---|
| **Assign the 3 crew-window rosters (load-in/event-run/load-out) against the current team** | 1032 | Needs real names next to the staffing matrix, not just role counts |
| **Set up the Mon/Wed/Fri posting cadence in Firefly (or whatever runs ZAO's social scheduling)** | 1033 | Should start immediately in practice, but the actual scheduling setup can follow once Tier 1-2 items are handled |
| **Run a full speed test at the actual Franklin Street Parklet venue** for the streaming connectivity plan | 1030 | Needs the venue and equipment in hand first - naturally sequenced after Tier 2's gear purchase |
| **Run a full end-to-end test stream** with all gear, ideally at the real venue, at least 2 weeks before Oct 3 | 1030 | The final verification step - only meaningful once everything above it is actually done |
| **Run a full walkthrough of the day-of operations timeline with the team**, at least 2 weeks before Oct 3 | 1032 | Same logic - this is the dress rehearsal, not a standalone task |

## What this doc deliberately does NOT include

- Doc 1011 (ZAO ecosystem database architecture) - real and important, but ecosystem-wide, not ZAOstock-specific. Its own Next Actions stand on their own.
- Anything from doc 1009 (zaofestivals.com brand audit) not directly touching ZAOstock's Oct 3 execution - that doc's actions are brand/domain work, a different track.
- The ZABAL Gamez / Loops judging-infrastructure idea Zaal raised mid-session - a real, separate thread, deliberately not folded in here since it's not part of Oct 3 execution.

## Also See

- [Doc 1007](../1007-zaostock-t86-readiness-audit/), [Doc 1013](../../business/1013-zaofestivals-budgets-zaostock-zaoville/), [Doc 1030](../1030-zaostock-livestream-media-production/), [Doc 1031](../../business/1031-zaostock-why-ellsworth-why-this-model/), [Doc 1032](../1032-zaostock-day-of-operations-plan/), [Doc 1033](../1033-zaostock-photo-promo-calendar/), [Doc 1034](../1034-zao-guides-acadia-ellsworth-excursions/) — the 7 source docs this punch list consolidates

## Next Actions

This doc's own action IS the punch list above - no separate table. The one meta-action:

| Action | Owner | Type | By When |
|--------|-------|------|---------|
| Review this doc weekly and cross off items as they close, rather than letting new research docs re-scatter the list | Zaal | Task | ongoing, first check-in 2026-07-18 |

## Sources

- Docs 1007, 1013, 1030, 1031, 1032, 1033, 1034 (all shipped this session) — [FULL, internal, direct synthesis of each doc's own Next Actions table]
