...
| **Write down who "Roddy" is** | The tracker has a real, dated action item - "ZAOstock: permit OR City co-sponsorship (call Roddy first)," due 2026-08-19, matching doc 1032's Aug 19 permit deadline exactly - but no research doc anywhere names who Roddy is or how to reach him. This is a real, specific lead sitting only in a one-line tracker task title |
...
Already flagged and logged as a tracker action item this session (`inbox:zaostock-tracker-empty-tables`) - restated here as part of the same fresh-check exercise doc 1035 asked for. Fellenz and Dcoop are confirmed live artists (doc 1033), Heart of Ellsworth is a confirmed venue/promotion partner (doc 1031), and doc 1013 reconciled a real $20K budget with a 5-line spending template - none of it exists as rows in `artists`/`sponsors`/`budget_entries`. This session's MCP connection is read-only, so the insert itself is still pending someone with write access. [FULL - direct table check this session, corroborates the inbox item already logged]
...
- **"ZAOstock: permit OR City co-sponsorship (call Roddy first)"** - due 2026-08-19, exactly matching doc 1032's Ordinance Chapter 14 deadline. "Roddy" is a real, specific named contact for the permit path that no research doc names or explains.
...
- [Doc 1013 — ZAO Festivals Budgets](../../business/1013-zaofestivals-budgets-zaostock-zaoville/) — source of the $20K budget line items still missing from `budget_entries`
...
| Find out who Roddy is and add a one-line note to doc 1032 or the tracker task itself | Zaal | Todo | 2026-07-18 |
