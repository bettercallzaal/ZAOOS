...
- [ ] ZAO free-tier Vercel dig - BLOCKED on Zaal: either `security find-generic-password -wa Arc` + Always Allow (then Claude imports Arc's vercel.com cookies via gstack /browse), or create a token at vercel.com/account/settings/tokens in Arc and stash at ~/.zao/private/vercel-zao.token (chmod 600). Goal: name which route burns 1.1M invocations + 6h Fluid CPU on the free account (suspect: bot polling of thezao.xyz API routes).
...
- [ ] Identify the $926/day Claude session - `zao-spend --by-lane` names the working dir; one ZAOOS session consumed $926 list-price in 24h (1.2M output tokens, 24M transcript). Decide if that loop's output is worth its burn.
...
- Open questions for the receiver: Which access route does Zaal prefer for the ZAO Vercel account (Keychain allow vs token)? Is the $926/day session a sanctioned loop?
