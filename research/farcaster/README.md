...
| 595 | [FIP Live Activity DEEPER — adjacent FIPs, 5-app coalition, JFS reference impl](./595-fip-live-activity-deeper/) | DECISION-DEEP | Third doc in FIP Live Activity stack. CRITICAL: FIP #262 explicitly PUNTS on per-`UserDataType` scopes — the gap I assumed it would close stays open. FIP #163 scaling is dying (8 months stalled). FIP #266 moving forward as meta-FIP. JFS reference implementation FULLY OPEN at `farcasterxyz/miniapps/packages/miniapp-node/src/jfs.ts` — uses `@noble/curves/ed25519`, ~100 lines, lift-ready TypeScript included. Coalition expands from 2 to 5 apps: Juke, Streamly (open source!), Livecaster, FarHouse, Fireside. Caught spec inconsistency: FIP #269 says `header.type='auth'` but reference impl uses `'app_key'` — flagged. 31 verified sources. |
