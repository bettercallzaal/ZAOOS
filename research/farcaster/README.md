...
| 596 | [FIP Live Activity QUILIBRIUM PRIVACY BRIDGE + impl blockers](./596-fip-live-activity-quilibrium-privacy/) | DECISION-DEEP | Fourth doc in FIP Live Activity stack. Maps Quilibrium primitives to FIP #268 risks: OT mitigates stalking (Risk 1), MPC mitigates co-location inference (Risk 2). Risk 3 (permanent history) is Snapchain immutability mismatch — would need off-chain LIVE_AT. CRITICAL impl blocker found: Snapchain enum doesn't yet include `USER_DATA_TYPE_LIVE_AT = 10`. PR #1, #2, #5 ship NOW (app-side only). PR #3, #4 wait on Snapchain. NEW PR #6 = Hypersnap-OT bridge for ZAO-gated room privacy. ZAO is the only ecosystem player with all 4 assets needed to bridge Quilibrium + FIP Live Activity. Engagement plan refined — Cassie engages via code not DMs, Hypersnap Issue #17 = warm intro, ship PR over weekend. 22 verified sources. |
