...
| Verify why ZOE recall returns empty (it is NOT a Ryan-flipped lever) | YES, BLOCKER | ZOE's `searchVectorStore()` returns `count: 0` and degrades to manual @zabal_bonfire relay (`bot/src/zoe/recall.ts:12-13`). Per Zaal: labeling/search is **internal to the Bonfires platform** - it runs server-side, NOT a manual `/labeling/hybrid` admin toggle Ryan controls. The `recall.ts:12-13` comment is likely stale. So the empty-recall cause is something else: wrong endpoint/payload, the bonfire not yet indexed, or an SDK-vs-REST mismatch. Diagnose against the live API, do not wait on Ryan to "run labeling". This still gates ALL automated read value. |
...
| Diagnose ZOE empty-recall against the LIVE Bonfires API - labeling is platform-internal (not a Ryan toggle, per Zaal). Check `/vector_store/search` payload, confirm the bonfire is indexed, test SDK `delve` vs raw REST. Update the stale `recall.ts:12-13` comment | @Zaal | Code/API debug | Next sprint |
