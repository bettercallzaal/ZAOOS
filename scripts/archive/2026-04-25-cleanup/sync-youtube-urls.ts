...
const TRANSCRIPTS_DIR = path.join(process.cwd(), 'content/transcripts/bcz-yapz');
const GAPS_LOG = path.join(process.cwd(), 'scripts/sync-youtube-urls.gaps.log');
...
function isYapzVideo(entry: YtEntry): boolean {
  ...
  return t.startsWith('bcz yapz') || t.startsWith('better call zaal yap') || t.includes(' yapz ');
}
...
function main() {
  ...
  const filenames = fs.readdirSync(TRANSCRIPTS_DIR).filter((f) => f.endsWith('.md'));
  ...
  if (gaps.length > 0) {
    fs.writeFileSync(GAPS_LOG, gaps.join('\n') + '\n', 'utf-8');
