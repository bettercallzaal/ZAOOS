    ...
    searchTimerRef.current = setTimeout(async () => {
      ...
      try {
        const res = await fetch(
          `/api/miniapp/search?q=${encodeURIComponent(discoverSearch.trim())}`,
        );
        const data = await res.json();
        const items: MiniApp[] =
          data?.frames ?? data?.mini_apps ?? data?.apps ?? data?.results ?? [];
    ...
    <div className="flex flex-col min-h-[100dvh] bg-[#0a1628] text-white">
              ...
              <div>
                ...
                {searchLoading ? (
                  ...
                  <p className="text-sm text-gray-600 py-6 text-center">
                    No results for &ldquo;{discoverSearch}&rdquo;
                  </p>
                ) : (
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
      ...
      <div className="flex-shrink-0 bg-gradient-to-r from-[#f5a623]/10 to-[#ffd700]/5 border-t border-[#f5a623]/20 px-4 py-2.5">
        <div className="flex items-center justify-between">
          <div>
            <span className="text-[10px] text-[#f5a623] font-medium uppercase tracking-wider">
              $ZABAL on Base
            </span>
            <p className="text-[10px] text-gray-500 mt-0.5">
              Community token launched via Clanker, Jan 1 2026
            </p>
          </div>
          <a
      ...
      <div className="flex-shrink-0 border-t border-white/[0.08] bg-[#0d1b2a]">
        ...
        >
          <span className="text-xs font-medium text-gray-400">
            More Resources ({allLinks.length} links)
          </span>
          <svg
              ...
              <div className="space-y-1.5">
                {filteredLinks.length === 0 && (
                  <p className="text-xs text-gray-600 py-4 text-center">
                    No results for &ldquo;{resourceSearch}&rdquo;
                  </p>
                )}
      ...
      <div className="flex items-start gap-2">
        {app.imageUrl ? (
          ...
          <div className="w-9 h-9 rounded-lg bg-[#1a3a5c] flex items-center justify-center flex-shrink-0">
            <svg
              className="w-4 h-4 text-[#60b4ff]/40"
              viewBox="0 0 24 24"
              fill="none"
              stroke="currentColor"
              strokeWidth={1.5}
        ...
        <div className="flex-1 min-w-0">
          ...
          {app.author && (
            <p className="text-[10px] text-gray-600 truncate">
              by {app.author.displayName || app.author.username}
            </p>
          )}
        </div>
      </div>
...
function ResourceLinkRow({ link }: { link: NexusLink }) {
  return (
    ...
    >
      ...
      <svg
        className="w-3 h-3 text-gray-700 group-hover:text-gray-400 flex-shrink-0"
        viewBox="0 0 24 24"
        fill="none"
        stroke="currentColor"
        strokeWidth={2}
