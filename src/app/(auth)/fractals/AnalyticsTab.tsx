    ...
    <div className="pt-2 space-y-5">
      ...
      <div className="grid grid-cols-2 gap-2">
        {[
          {
            label: 'Total Respect',
            value: overview.totalRespect.toLocaleString(),
            sub: 'All sources combined',
          },
          {
            label: 'Sessions',
            value: overview.totalSessions.toString(),
            sub: `${overview.ogSessions} OG + ${overview.ordaoSessions} ORDAO`,
          },
          {
            label: 'Participants',
            value: overview.uniqueParticipants.toString(),
            sub: `${overview.totalMembers} total members`,
          },
          {
            label: 'On-Chain',
            value: `${overview.totalOGOnchain.toLocaleString()} OG`,
            sub: `${overview.totalZOROnchain.toLocaleString()} ZOR`,
          },
        ].map((stat) => (
      ...
      <div className="bg-[#0d1b2a] rounded-xl p-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="h-1.5 bg-[#0a1628] rounded-full mt-0.5 overflow-hidden">
                    <div className="h-full rounded-full flex">
                      <div
                        className="bg-[#f5a623]"
                        style={{ width: `${(member.fractal / maxTotal) * 100}%` }}
                      />
                      <div
                        className="bg-[#f5a623]/40"
                        style={{ width: `${(member.events / maxTotal) * 100}%` }}
                      />
                      <div
                        className="bg-[#f5a623]/20"
                        style={{ width: `${((member.hosting + member.bonus) / maxTotal) * 100}%` }}
                      />
                    </div>
                  </div>
                </div>
        ...
        <div className="flex gap-3 mt-3 text-[10px] text-gray-600">
          <span className="flex items-center gap-1">
            <span className="w-2 h-2 rounded-full bg-[#f5a623]" /> Fractal
          </span>
          <span className="flex items-center gap-1">
            <span className="w-2 h-2 rounded-full bg-[#f5a623]/40" /> Events
          </span>
          <span className="flex items-center gap-1">
            <span className="w-2 h-2 rounded-full bg-[#f5a623]/20" /> Hosting/Bonus
          </span>
        </div>
      </div>
          ...
          <div className="bg-[#0d1b2a] w-full sm:max-w-lg sm:rounded-xl rounded-t-xl max-h-[85vh] overflow-y-auto border border-white/[0.08]">
            ...
            {memberLoading ? (
              <div className="p-4 space-y-2">
                {[1, 2, 3].map((i) => (
                  <div key={i} className="h-10 bg-[#0a1628] rounded animate-pulse" />
                ))}
              </div>
            ) : memberProfile?.stats ? (
                ...
                <div className="grid grid-cols-3 gap-2">
                  <div className="bg-[#0a1628] rounded-lg p-2 text-center">
                    <p className="text-lg font-bold text-[#f5a623]">
                      {memberProfile.stats.totalSessions}
                    </p>
                    <p className="text-[10px] text-gray-500">Sessions</p>
                  </div>
                  <div className="bg-[#0a1628] rounded-lg p-2 text-center">
                    <p className="text-lg font-bold text-[#f5a623]">
                      {memberProfile.stats.totalFractalRespect}
                    </p>
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <span className="text-[10px] text-gray-600">
                              {h.sessionDate || ''}
                              {h.participants ? ` · ${h.participants} participants` : ''}
                              {h.era ? ` · ${h.era}` : ''}
                            </span>
                          </div>
                ...
                {memberProfile.ledger && memberProfile.ledger.length > 0 && (
                  <div>
                    ...
                    <p className="text-[10px] text-gray-600 mb-2">
                      Every respect point earned — when, how, and why
                    </p>
                    <div className="space-y-1 max-h-72 overflow-y-auto">
                      {memberProfile.ledger.map(
                        (
                          entry: {
                            date: string | null;
                            source: string;
                          ...
                          >
                            ...
                            <span className="font-mono text-green-400 flex-shrink-0">
                              +{entry.amount}
                            </span>
                          </div>
                        ),
                    ...
                    <p className="text-[10px] text-gray-600 mt-2">
                      Total:{' '}
                      {memberProfile.ledger.reduce(
                        (sum: number, e: { amount: number }) => sum + e.amount,
                        0,
                      )}{' '}
                      respect from {memberProfile.ledger.length} entries
                    </p>
                  </div>
                )}
