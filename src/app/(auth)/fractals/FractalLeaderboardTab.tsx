...
export function FractalLeaderboardTab({ currentFid, onViewMember }: Props) {
  ...
  const filtered = useMemo(() => {
    ...
    if (search) {
      ...
      list = list.filter(
        (e) => e.name.toLowerCase().includes(q) || e.wallet?.toLowerCase().includes(q),
      );
    }
  ...
  return (
    <div className="pt-2 space-y-3">
          ...
          <div className="h-2 bg-[#0a1628] rounded-full overflow-hidden flex">
            ...
            {me.hostingRespect + me.bonusRespect > 0 && (
              <div
                ...
                style={{
                  width: `${((me.hostingRespect + me.bonusRespect) / Math.max(me.totalRespect, 1)) * 100}%`,
                }}
              />
            )}
          </div>
      ...
      <div className="space-y-1">
        {filtered.map((entry) => {
          ...
          return (
            <div
              key={entry.name}
              onClick={onViewMember ? () => onViewMember(entry.name) : undefined}
              onKeyDown={
                onViewMember
                  ? (ev) => {
                      if (ev.key === 'Enter' || ev.key === ' ') {
                        ev.preventDefault();
                        onViewMember(entry.name);
                      }
                    }
                  : undefined
              }
              ...
              className={`flex items-center gap-2 px-3 py-2 rounded-xl transition-colors ${
                onViewMember ? 'cursor-pointer hover:bg-[#f5a623]/10' : ''
      ...
      <p className="text-[10px] text-gray-600 text-center">
        Showing {filtered.length} of {totalMembers} members. Respect = Fractal + Events + Hosting +
        Bonus.
      </p>
    </div>
  );
