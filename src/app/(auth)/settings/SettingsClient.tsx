...
const CrossfadeSettings = dynamic(
  () =>
    import('@/components/music/CrossfadeSettings').then((m) => ({ default: m.CrossfadeSettings })),
  { ssr: false },
);
const LastfmConnect = dynamic(
  () => import('./LastfmConnect').then((m) => ({ default: m.LastfmConnect })),
  { ssr: false },
);
const ListenBrainzConnect = dynamic(
  () => import('./ListenBrainzConnect').then((m) => ({ default: m.ListenBrainzConnect })),
  { ssr: false },
);
const SocialsSection = dynamic(
  () => import('./SocialsSection').then((m) => ({ default: m.SocialsSection })),
  { ssr: false },
);
const CrossPostingSection = dynamic(
  () => import('./CrossPostingSection').then((m) => ({ default: m.CrossPostingSection })),
  { ssr: false },
);
...
const FacebookConnect = dynamic(() => import('@/components/settings/FacebookConnect'), {
  ssr: false,
});
  ...
  return (
    ...
    >
      <path
        ...
        d="M21 12a2.25 2.25 0 00-2.25-2.25H15a3 3 0 110-6h5.25A2.25 2.25 0 0121 6v6zm0 0v6a2.25 2.25 0 01-2.25 2.25H5.25A2.25 2.25 0 013 18V6a2.25 2.25 0 012.25-2.25h13.5A2.25 2.25 0 0121 6m-7.5 6a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0z"
      />
    </svg>
  );
}
      ...
      <div className="bg-[#0d1b2a] rounded-xl p-4 border border-white/[0.08] space-y-4">
        ...
        <div>
          ...
          <p className="text-xs text-gray-500 mt-0.5">
            Copy this URL into OBS as a Browser Source to show what you&apos;re playing.
          </p>
          <div className="flex items-center gap-2 mt-2">
            <code className="flex-1 text-[11px] text-gray-300 bg-[#0a1628] rounded px-2 py-1.5 truncate border border-white/[0.08]">
              {overlayUrl}
            </code>
            <button
              onClick={() => copyUrl(overlayUrl, 'np')}
              className="px-2.5 py-1.5 text-xs rounded bg-[#f5a623]/10 text-[#f5a623] hover:bg-[#f5a623]/20 transition-colors flex-shrink-0"
            >
          ...
          <div className="flex items-center gap-1.5 mt-2">
            ...
            {OVERLAY_STYLES.map((s) => (
              <button
                key={s}
                onClick={() => setStyle(s)}
                className={`px-2 py-0.5 text-[10px] rounded-full capitalize transition-colors ${style === s ? 'bg-[#f5a623] text-black font-medium' : 'bg-gray-800 text-gray-400 hover:text-white'}`}
              >
                {s}
              </button>
            ))}
          ...
          <div className="flex items-center gap-1.5 mt-2">
            ...
            {(['small', 'medium', 'large'] as const).map((s) => (
              <button
                key={s}
                onClick={() => setSize(s)}
                className={`px-2 py-0.5 text-[10px] rounded-full capitalize transition-colors ${size === s ? 'bg-[#f5a623] text-black font-medium' : 'bg-gray-800 text-gray-400 hover:text-white'}`}
              >
                {s}
              </button>
            ))}
          ...
          <button
            ...
            className="text-[10px] text-[#f5a623] hover:underline mt-2"
          >
          ...
          {showPreview && (
            <iframe
              src={overlayUrl}
              className="w-[300px] h-[200px] rounded border border-white/[0.08] mt-2"
        ...
        {[
          {
            label: 'Room Chat Overlay',
            url: `/overlay/chat?roomId=...`,
            desc: 'Show live room chat on stream',
          },
          {
            label: 'Recent Tips Overlay',
            url: `/overlay/tips?fid=${fid}`,
            desc: 'Display recent tip activity',
          },
        ].map((item) => (
          <div key={item.label} className="opacity-50">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[10px] text-gray-600 border border-white/[0.08] rounded px-1.5 py-0.5">
                Coming Soon
              </span>
            </div>
          </div>
...
export function SettingsClient({ session, profile }: SettingsClientProps) {
  ...
  const togglePref = useCallback(
    async (key: keyof MessagingPrefs) => {
  ...
  const toggleWalletVisibility = useCallback(
    async (key: WalletKey) => {
      ...
      const updated = isHidden ? hiddenWallets.filter((k) => k !== key) : [...hiddenWallets, key];
  ...
  const togglePushNotifications = async () => {
    ...
    try {
      ...
      if (!inMiniApp) {
        ...
        alert(
          'Push notifications can only be enabled when using ZAO OS as a Farcaster Mini App. Open ZAO OS from Warpcast to enable.',
        );
  ...
  const accountConnections = [!!session?.walletAddress, !!session?.fid, !!solanaWallet];
      ...
      <div className="max-w-lg mx-auto px-4 py-6 space-y-6">
        ...
        <section>
          <div className="flex items-center justify-between px-1 mb-3">
            ...
            <span className="text-[10px] text-gray-600">
              {accountConnectedCount} of {accountConnections.length} connected
            </span>
          </div>
          <div className="bg-[#0d1b2a] rounded-xl px-4 border border-white/[0.08] divide-y divide-gray-800/50">
            ...
            <AccountRow
              ...
              detail={
                session.fid ? `@${profile?.username || `FID ${session.fid}`}` : 'Not connected'
              }
              action={
                session.signerUuid ? (
            ...
            {/* 3. Solana */}
            <div className="py-3">
              <LazySolanaWalletConnect savedWallet={solanaWallet} onSaved={setSolanaWallet} />
            </div>
          </div>
        </section>
          ...
          <div className="bg-[#0d1b2a] rounded-xl p-5 border border-white/[0.08]">
            <div className="flex items-center gap-4 mb-4">
              {profile.pfp_url ? (
                <Image
                  src={profile.pfp_url}
                  alt={profile.fc_display_name || 'avatar'}
                  width={56}
                  height={56}
                ...
                <div className="w-14 h-14 rounded-full bg-gray-700 flex items-center justify-center">
                  <span className="text-lg text-gray-400 font-bold">
                    {(profile.fc_display_name || '?')[0]?.toUpperCase()}
                  </span>
                </div>
              )}
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <p className="text-lg font-bold text-white truncate">
                    {profile.fc_display_name || 'Anonymous'}
                  </p>
                  {profile.power_badge && (
            ...
            <div className="mt-3 pt-3 border-t border-white/[0.08]">
              <ShareToFarcaster
                template={shareTemplates.profile(
                  profile.zid,
                  profile.fc_display_name || profile.zao_display_name || 'ZAO Member',
                )}
                variant="button"
            ...
            {zaoEditing ? (
              ...
              <>
                ...
                <div className="px-5 py-4">
                  ...
                  <p className="text-sm text-white mt-0.5">
                    {zaoFields.bio || <span className="text-gray-600 italic">Not set</span>}
                  </p>
                </div>
                <div className="px-5 py-4">
                  <p className="text-xs text-gray-500">IGN (In-Game Name)</p>
                  <p className="text-sm text-white mt-0.5">
                    {zaoFields.ign || <span className="text-gray-600 italic">Not set</span>}
                  </p>
                </div>
                <div className="px-5 py-4">
                  <p className="text-xs text-gray-500">Real Name</p>
                  <p className="text-sm text-white mt-0.5">
                    {zaoFields.real_name || <span className="text-gray-600 italic">Not set</span>}
                  </p>
                </div>
                <div className="px-5 py-3">
                  <p className="text-[10px] text-gray-600">
                    ZAO profile fields supplement your Farcaster identity within the community.
                  </p>
                </div>
              </>
            )}
            ...
            {respectLoading ? (
              ...
              <>
                ...
                {respect.member.first_respect_at && (
                  <div className="mt-3 pt-3 border-t border-white/[0.08]">
                    <p className="text-[10px] text-gray-600">
                      First respect earned{' '}
                      {new Date(respect.member.first_respect_at).toLocaleDateString('en-US', {
                        month: 'short',
                        day: 'numeric',
                        year: 'numeric',
                      })}
                ...
                <div className="mt-3 pt-3 border-t border-white/[0.08]">
                  <Link
                    href="/respect"
                    className="text-xs text-[#f5a623]/60 hover:text-[#f5a623] transition-colors"
                  >
              ...
              <div className="text-center py-4">
                ...
                <p className="text-xs text-gray-600 mt-1">
                  Earn respect by attending Fractal sessions
                </p>
              </div>
            )}
          </div>
          ...
          <div className="bg-[#0d1b2a] rounded-xl border border-white/[0.08] divide-y divide-gray-800/50">
            ...
            <div className="flex items-center justify-between px-5 py-4">
              <div className="flex-1 min-w-0 pr-3">
                ...
                <p className="text-xs text-gray-500">
                  Automatically join the ZAO General group when messaging is enabled
                </p>
              </div>
              <button
            ...
            <div className="flex items-center justify-between px-5 py-4">
              <div className="flex-1 min-w-0 pr-3">
                ...
                <p className="text-xs text-gray-500">
                  Accept direct messages from people outside the ZAO community
                </p>
              </div>
              <button
        ...
        <section>
          ...
          <div className="bg-[#0d1b2a] rounded-xl p-5 border border-white/[0.08]">
            <div className="flex items-center gap-3 mb-3">
              <div
                className={`w-10 h-10 rounded-xl flex items-center justify-center ${hasSigner ? 'bg-green-500/10' : 'bg-yellow-500/10'}`}
              >
                {hasSigner ? (
                  <svg
            ...
            {hasSigner ? (
              <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-green-500/5 border border-green-500/10">
                ...
                <span className="text-xs text-green-500/80">
                  Signer active — {shortAddr(session.signerUuid || '')}
                </span>
              </div>
            ) : (
              <div>
        ...
        <section>
          ...
          <div className="bg-[#0d1b2a] rounded-xl border border-white/[0.08] divide-y divide-gray-800/50">
                  ...
                  {profile.verified_addresses.map((addr) => {
                    ...
                    return (
                      <div key={addr} className="flex items-center justify-between gap-2">
                        <div className="flex-1 min-w-0">
                          <p
                            className={`text-sm font-mono truncate ${isHidden ? 'text-gray-600' : 'text-white'}`}
                          >
                            {addr}
                          </p>
                        ...
                        <button
                          ...
                          title={
                            isHidden
                              ? 'Hidden from profile — tap to show'
                              : 'Visible on profile — tap to hide'
                          }
                        >
                          {isHidden ? (
                            <svg
            ...
            <div className="px-5 py-3">
              <p className="text-[10px] text-gray-600">
                Toggle the eye icon to show or hide each wallet on your public profile. Hidden
                wallets are still visible to you here.
              </p>
            </div>
          </div>
        </section>
            ...
            {storageLoading ? (
              ...
              <div className="space-y-4">
                  ...
                  return (
                    <div key={label}>
                      <div className="flex items-center justify-between mb-1.5">
                        ...
                        <span
                          className={`text-xs font-mono ${isWarning ? 'text-[#f5a623]' : 'text-gray-500'}`}
                        >
                          {data.used.toLocaleString()} / {data.capacity.toLocaleString()}
                          {isWarning && (
                            <span className="ml-1.5 text-[#f5a623]">({pct}% full)</span>
                          )}
                        </span>
                      </div>
                ...
                <p className="text-[10px] text-gray-600 pt-1">
                  Storage units are allocated per Farcaster account. Each unit holds data for one of
                  your actions.
                </p>
              </div>
            )}
          ...
          <div className="bg-[#0d1b2a] rounded-xl border border-white/[0.08] divide-y divide-gray-800/50">
            <div className="flex items-center justify-between px-5 py-4">
              ...
              <span className="text-xs text-gray-600 font-mono">
                {session.walletAddress ? shortAddr(session.walletAddress) : `FID ${session.fid}`}
              </span>
            </div>
