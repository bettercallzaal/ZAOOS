...
export async function GET() {
  ...
  try {
    ...
    const { data: users } = await supabaseAdmin
      .from('users')
      .select(
        'id, fid, username, display_name, primary_wallet, member_tier, respect_member_id, community_profile_id, discord_id, zid, real_name, ens_name, bluesky_handle, x_handle, preferred_wallet, last_login_at, last_active_at, role, is_active',
      )
      .eq('is_active', true);
    ...
    const { data: respectMembers } = await supabaseAdmin
      ...
      .select(
        'id, name, wallet_address, fid, total_respect, onchain_og, onchain_zor, fractal_count',
      );
    ...
    for (const u of allUsers) {
      if (!u.fid) {
        issues.push({
          severity: 'high',
          member: u.display_name || u.primary_wallet || u.id,
          issue: 'No Farcaster FID linked',
          fix: 'Link Farcaster account in admin panel',
        });
      }
    }
    ...
    for (const u of allUsers) {
      if (!u.respect_member_id) {
        ...
        if (match) {
          issues.push({
            severity: 'medium',
            member: u.display_name || u.username || '?',
            issue: 'Respect data exists but not linked',
            fix: `Link respect_member_id = ${match.id}`,
          });
        }
      }
    }
    ...
    for (const r of allRespect) {
      ...
      if (!hasUser && (r.total_respect || 0) > 0) {
        issues.push({
          severity: 'medium',
          member: r.name,
          issue: `Has ${r.total_respect} respect but no ZAO OS account`,
          fix: 'Add to allowlist or invite to sign up',
        });
      }
    }
    ...
    for (const u of allUsers) {
      if (u.member_tier === 'community') {
        ...
        if (
          respect &&
          ((respect.onchain_og || 0) > 0 ||
            (respect.onchain_zor || 0) > 0 ||
            (respect.total_respect || 0) > 0)
        ) {
          issues.push({
            severity: 'high',
            member: u.display_name || u.username || '?',
            issue: `Has ${respect.total_respect} respect but tier is "community"`,
            fix: 'Update to respect_holder',
          });
        }
      }
    }
    ...
    for (const u of allUsers) {
      ...
      if (missing.length >= 3) {
        issues.push({
          severity: 'low',
          member: u.display_name || u.username || u.primary_wallet || '?',
    ...
    for (const a of allAllowlist) {
      ...
      if (!hasUser) {
        issues.push({
          severity: 'low',
          member: a.real_name || a.wallet_address || '?',
          issue: 'On allowlist but never logged in',
        });
      }
    }
    ...
    const stats = {
      ...
      unlinkedRespect: allRespect.filter(
        (r) =>
          !allUsers.some(
            (u) =>
              (u.fid && r.fid === u.fid) ||
      ...
      allowlistNotLoggedIn: allAllowlist.filter(
        (a) =>
          !allUsers.some(
            (u) =>
              (a.fid && u.fid === a.fid) ||
              (a.wallet_address &&
                u.primary_wallet?.toLowerCase() === a.wallet_address.toLowerCase()),
          ),
      ).length,
