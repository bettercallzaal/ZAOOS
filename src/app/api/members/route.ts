...
export async function GET() {
  ...
  try {
    const { data, error } = await supabaseAdmin
      .from('allowlist')
      .select(
        'id, fid, username, real_name, ign, display_name, pfp_url, wallet_address, custody_address, verified_addresses',
      )
      .eq('is_active', true)
    ...
    const needsEnrich = (data || []).filter(
      (m) =>
        ...
        (!m.pfp_url ||
          !m.username ||
          !m.custody_address ||
          !m.verified_addresses ||
          (m.verified_addresses as string[]).length === 0),
    );
    ...
    if (needsEnrich.length > 0) {
      ...
      const backfillPromises = needsEnrich
        ...
        .map((m) => {
          ...
          if (
            enriched.verified_addresses.length > 0 &&
            (!m.verified_addresses || (m.verified_addresses as string[]).length === 0)
          ) {
          ...
          return supabaseAdmin
            ...
            .then(({ error: backfillError }) => {
              if (backfillError)
                logger.error('[members] backfill error for id', m.id, backfillError);
            });
        });
