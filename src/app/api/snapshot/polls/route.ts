...
export async function GET() {
  try {
    const [active, recent] = await Promise.allSettled([fetchActivePolls(), fetchRecentPolls()]);
    ...
    return NextResponse.json({ error: 'Failed to fetch Snapshot polls' }, { status: 500 });
  }
}
