...
export async function GET(request: NextRequest) {
  ...
  try {
    const channel = request.nextUrl.searchParams.get('channel')?.trim() || DEFAULT_CHANNEL;
    ...
    return NextResponse.json({ error: 'Failed to fetch trending rankings' }, { status: 500 });
  }
}
