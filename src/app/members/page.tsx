...
export default function MembersDirectoryPage() {
  ...
  const [filterOptions, setFilterOptions] = useState<{ categories: string[]; locations: string[] }>({ categories: [], locations: [] });
            ...
            <div className="flex gap-3 flex-wrap">
              <div className="flex-1 min-w-[120px]">
                ...
                <select
                  ...
                  onChange={e => setFilters(f => ({ ...f, active_since: e.target.value as Filters['active_since'] }))}
                  className="w-full bg-[#0a1628] border border-gray-700/50 rounded-lg px-2 py-1.5 text-[11px] text-white"
                >
              ...
              <div className="flex-1 min-w-[120px]">
                ...
                <select
                  ...
                  onChange={e => setFilters(f => ({ ...f, min_respect: Number(e.target.value) }))}
                  className="w-full bg-[#0a1628] border border-gray-700/50 rounded-lg px-2 py-1.5 text-[11px] text-white"
                >
            ...
            <div className="flex gap-3 flex-wrap items-center">
              <label className="flex items-center gap-1.5 text-[11px] text-gray-400 cursor-pointer">
                <input
                  ...
                  onChange={e => setFilters(f => ({ ...f, has_ens: e.target.checked }))}
          ...
          <div className="flex flex-wrap gap-1.5 mb-3">
            {filters.tier !== 'all' && (
              <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-[#f5a623]/10 text-[#f5a623] text-[10px]">
                ...
                <button onClick={() => setFilters(f => ({ ...f, tier: 'all' }))} className="hover:text-white">&times;</button>
              </span>
            )}
            {filters.category && (
              <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-purple-500/10 text-purple-400 text-[10px] capitalize">
                {filters.category}
                <button onClick={() => setFilters(f => ({ ...f, category: '' }))} className="hover:text-white">&times;</button>
              </span>
            )}
            {filters.platform && (
              <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-green-500/10 text-green-400 text-[10px] capitalize">
                On {filters.platform}
                <button onClick={() => setFilters(f => ({ ...f, platform: '' }))} className="hover:text-white">&times;</button>
              </span>
            )}
            {filters.active_since && (
              <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-blue-500/10 text-blue-400 text-[10px]">
                ...
                <button onClick={() => setFilters(f => ({ ...f, active_since: '' }))} className="hover:text-white">&times;</button>
              </span>
            )}
            {filters.min_respect > 0 && (
              <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-[#f5a623]/10 text-[#f5a623] text-[10px]">
                {filters.min_respect}+ Respect
                <button onClick={() => setFilters(f => ({ ...f, min_respect: 0 }))} className="hover:text-white">&times;</button>
              </span>
            )}
            {filters.has_ens && (
              <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-[#f5a623]/10 text-[#f5a623] text-[10px]">
                Has ENS
                <button onClick={() => setFilters(f => ({ ...f, has_ens: false }))} className="hover:text-white">&times;</button>
              </span>
            )}
            {filters.featured && (
              <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-[#f5a623]/10 text-[#f5a623] text-[10px]">
                Featured
                <button onClick={() => setFilters(f => ({ ...f, featured: false }))} className="hover:text-white">&times;</button>
              </span>
            )}
