...
export function DiscordLinkManager() {
  ...
  const [bulkStep, setBulkStep] = useState<
    'idle' | 'loading-file' | 'previewing' | 'executing' | 'done'
  >('idle');
    ...
    reader.onload = (ev) => {
      try {
        ...
        if (typeof data === 'object' && !Array.isArray(data)) {
          ...
          showFeedback(
            'error',
            'Invalid format. Expected JSON object: { discord_id: wallet_address }',
          );
          setBulkStep('idle');
        }
      } catch {
      ...
      <div className="bg-[#1a2a3a] rounded-xl p-4 mb-4 border border-white/[0.08]">
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              {walletMap
                ? `${Object.keys(walletMap).length} wallets loaded`
                : 'Upload wallets.json'}
            </button>
            ...
            {bulkPreview.matches.length > 0 && (
              <div className="max-h-40 overflow-y-auto space-y-1 mb-3">
                {bulkPreview.matches.map((m) => (
                  <div
                    key={m.userId}
                    className="flex items-center gap-2 text-xs bg-[#0a1628] rounded-lg px-3 py-1.5"
                  >
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      {user.fid && (
                        <span className="text-[10px] text-gray-600">FID:{user.fid}</span>
                      )}
                    </div>
                    <div className="flex items-center gap-2 mt-0.5">
                      ...
                      {user.discord_id && (
                        <span className="text-[10px] text-[#5865F2] font-mono">
                          {user.discord_id}
                        </span>
                      )}
                    </div>
                ...
                {isExpanded && (
                  <div className="px-4 pb-4 border-t border-white/[0.08]">
                    <div className="mt-3 space-y-2 text-xs">
                      ...
                      {/* Discord ID */}
                      <div className="flex items-center gap-2">
                        <span className="text-[10px] px-1.5 py-0.5 rounded bg-[#5865F2]/10 text-[#5865F2] font-medium w-16 text-center">
                          Discord
                        </span>
                      ...
                      {user.discord_id && (
                        <div className="flex items-center gap-4 mt-1">
                          <div className="flex items-center gap-1">
                            <span
                              className={`w-2 h-2 rounded-full ${user.has_intro ? 'bg-green-400' : 'bg-gray-600'}`}
                            />
                            <span className="text-gray-400">
                              {user.has_intro ? 'Has intro' : 'No intro'}
                            </span>
                          </div>
                    ...
                    <div className="flex items-center gap-2 mt-3 pt-3 border-t border-white/[0.08]">
                      {user.discord_id ? (
                        ...
                        <div className="flex items-center gap-2 flex-1">
                          <input
                            ...
                            onKeyDown={(e) => {
                              if (e.key === 'Enter' && linkInput.trim())
                                handleLink(user.id, linkInput.trim());
                              if (e.key === 'Escape') {
                                setLinkingId(null);
                                setLinkInput('');
                              }
                          ...
                          <button
                            onClick={() => {
                              if (linkInput.trim()) handleLink(user.id, linkInput.trim());
                            }}
