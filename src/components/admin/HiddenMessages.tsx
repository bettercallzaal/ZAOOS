...
import type { HiddenMessage } from '@/types';
      ...
      {messages.length === 0 ? (
        ...
        <div className="space-y-2">
          {messages.map((msg) => (
            <div
              key={msg.id}
              className="bg-[#1a2a3a] rounded-xl p-4 hover:bg-[#1e3048] transition-colors"
            >
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  ...
                  {msg.reason && <p className="text-gray-300 text-sm mt-1">{msg.reason}</p>}
                </div>
