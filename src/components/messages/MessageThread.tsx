      ...
      {[...Array(6)].map((_, i) => {
        ...
        return (
          <div key={i} className={`flex ${isRight ? 'justify-end' : 'justify-start'}`}>
            {!isRight && (
              <div className="w-8 h-8 rounded-full bg-gray-800/60 flex-shrink-0 mr-2.5" />
            )}
            <div className={`${isRight ? 'max-w-[60%]' : 'max-w-[65%]'}`}>
              {!isRight && <div className="w-20 h-2.5 bg-gray-800/40 rounded mb-2 ml-1" />}
              <div
                className={`rounded-2xl px-4 py-3 ${isRight ? 'bg-[#f5a623]/10' : 'bg-gray-800/30'}`}
              >
                <div className="space-y-1.5">
                  <div
                    className={`h-3 rounded ${isRight ? 'bg-[#f5a623]/15' : 'bg-gray-700/40'}`}
                    style={{ width: `${70 + ((i * 13) % 30)}%` }}
                  />
                  {i % 2 === 0 && (
                    <div
                      className={`h-3 rounded ${isRight ? 'bg-[#f5a623]/10' : 'bg-gray-700/30'}`}
              ...
              <div
                className={`w-10 h-2 bg-gray-800/20 rounded mt-1.5 ${isRight ? 'ml-auto mr-1' : 'ml-1'}`}
              />
            </div>
          </div>
        );
        ...
        <div className="text-center px-6 max-w-sm">
          ...
          {xmtpState === 'idle' && (
            <>
              <div className="w-20 h-20 mx-auto mb-5 rounded-2xl bg-gradient-to-br from-[#f5a623]/15 to-[#f5a623]/5 flex items-center justify-center border border-[#f5a623]/10">
                <svg
                  className="w-10 h-10 text-[#f5a623]/70"
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="currentColor"
                  strokeWidth={1.5}
              ...
              <p className="text-sm text-gray-400 leading-relaxed">
                Enable messaging in the sidebar to start encrypted conversations with ZAO members
              </p>
            </>
          )}
          ...
          {xmtpState === 'connecting' && (
            <>
              ...
              <p className="text-sm text-gray-400 leading-relaxed">
                Setting up your encrypted inbox... This may take a moment on first use.
              </p>
            </>
          )}
          ...
          {xmtpState === 'error' && (
            <>
              ...
              <p className="text-sm text-red-400/80 leading-relaxed mb-4">
                {xmtpError || 'Unable to connect to XMTP. Please try again.'}
              </p>
              {onRetryConnect && (
                <button
          ...
          {xmtpState === 'connected' && (
            <>
              ...
              <div className="flex items-center gap-3 mb-5">
                <div className="flex-1 border-t border-white/[0.08]/60" />
              ...
              <div className="flex flex-col gap-2.5">
                ...
                {onNewGroup && (
                  <button
                    onClick={onNewGroup}
                    className="flex items-center justify-center gap-2 w-full px-4 py-2.5 rounded-lg border border-white/[0.08] text-gray-300 text-sm font-medium hover:bg-white/5 hover:border-gray-600 transition-colors"
                  >
                    <svg
                      className="w-4 h-4"
                      viewBox="0 0 24 24"
      ...
      <div className="flex items-center justify-center gap-2 py-2.5 px-4 bg-green-500/5 border-b border-green-500/10">
        ...
        {conversation.type === 'dm' && (
          <span className="text-[11px] text-gray-600 ml-1">
            · Only you and {conversation.peerDisplayName || 'this person'} can read these messages
          </span>
        )}
      </div>
      ...
      <div className="px-3 sm:px-6 py-4">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center h-full text-center px-6">
            <div className="w-16 h-16 mx-auto mb-4 rounded-2xl bg-gradient-to-br from-[#1a2a3a] to-[#0d1b2a] border border-white/[0.08] flex items-center justify-center">
              <svg
                className="w-8 h-8 text-[#f5a623]/50"
                viewBox="0 0 24 24"
                fill="none"
                stroke="currentColor"
                strokeWidth={1.5}
            ...
            {messages.map((msg, i) => {
              ...
              return (
                <div key={msg.id}>
                  ...
                  <div
                    className={`flex ${msg.isFromMe ? 'justify-end' : 'justify-start'} ${isConsecutive ? 'mt-0.5' : 'mt-4'}`}
                  >
                    {/* Sender avatar */}
                    {!msg.isFromMe && !isConsecutive && (
                      <div className="w-8 h-8 flex-shrink-0 mr-2.5 mt-0.5">
                        {msg.senderPfpUrl ? (
                          ...
                          <div className="w-8 h-8 rounded-full bg-gradient-to-br from-[#1a2a3a] to-[#0d1b2a] border border-white/[0.08] flex items-center justify-center">
                    ...
                    {!msg.isFromMe && isConsecutive && <div className="w-8 flex-shrink-0 mr-2.5" />}
                    <div className="max-w-[80%] sm:max-w-[70%]">
                      ...
                      <div
                        className={`px-3.5 py-2 ${
                          ...
                          msg.isFromMe
                            ...
                            : 'bg-[#1a2a3a] text-gray-100 border border-white/[0.08]/30'
                        }`}
                      >
                      ...
                      <p
                        className={`text-[10px] mt-1 ${msg.isFromMe ? 'text-right mr-1' : 'ml-1'} text-gray-600`}
                      >
