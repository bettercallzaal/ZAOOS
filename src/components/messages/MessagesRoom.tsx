...
export function MessagesRoom() {
  ...
  useEffect(() => {
    ...
    const hasPreviousConnection =
      typeof window !== 'undefined' && localStorage.getItem('zaoos-xmtp-wallets');
  ...
  const handleCreateDm = useCallback(
    async (address: `0x${string}`, peerProfile?: XMTPPeerProfile) => {
  ...
  const handleCreateGroup = useCallback(
    async (name: string, members: { address: `0x${string}`; profile?: XMTPPeerProfile }[]) => {
    ...
    return (
      <div className="flex h-[100dvh] bg-[#0a1628] text-white">
        ...
        <ConnectXMTP
          ...
          error={error}
          onConnect={handleConnectWithWallet}
        />
      </div>
    );
  }
  ...
  return (
    <div className="flex h-[100dvh] bg-[#0a1628] text-white overflow-hidden pb-20 md:pb-0">
      ...
      {showList && (
        <div
          className={`${isMobile ? 'w-full' : 'w-80 border-r border-white/[0.08]'} bg-[#0d1b2a] flex flex-col`}
        >
                ...
                <div className="w-9 h-9 rounded-full bg-gray-700 flex items-center justify-center flex-shrink-0">
                  {activeConversation.type === 'group' ? (
                    ...
                    <span className="text-sm text-gray-400 font-medium">
                      {(activeConversation.peerDisplayName ||
