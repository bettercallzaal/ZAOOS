...
import type { Playlist, PlaylistMember, Track } from './CollaborativePlaylists';
  ...
  return (
    <div className="flex items-center gap-3 py-3 px-3 rounded-lg hover:bg-white/5 transition-colors group">
      <button
        ...
        className="relative w-10 h-10 flex-shrink-0 rounded-md overflow-hidden bg-white/10"
      >
        {track.song_artwork_url ? (
          <Image
            src={track.song_artwork_url || '/default-track.png'}
            alt=""
            width={40}
            height={40}
      ...
      <div className="flex items-center gap-1">
        ...
        <span
          className={`text-xs font-medium min-w-[20px] text-center ${track.votes > 0 ? 'text-[#f5a623]' : track.votes < 0 ? 'text-red-400' : 'text-gray-500'}`}
        >
          {track.votes}
        </span>
        <button
...
export function PlaylistDetail({ playlist, onBack }: { playlist: Playlist; onBack: () => void }) {
  ...
  useEffect(() => {
    ...
    setIsMember(
      !!myFid && Array.isArray(playlist.member_fids) && playlist.member_fids.includes(myFid),
    );
  ...
  const handleJoin = async () => {
    setJoining(true);
    try {
      const res = await fetch(`/api/music/playlists/collaborative/${playlist.id}/join`, {
        method: 'POST',
      });
      if (res.ok) {
        setIsMember(true);
      ...
      <div className="flex items-center gap-2 mb-4">
        ...
        {!isMember && (
          <button
            onClick={handleJoin}
            disabled={joining}
            className="px-3 py-1.5 rounded-lg bg-[#f5a623]/10 text-[#f5a623] text-xs font-medium hover:bg-[#f5a623]/20 transition-colors disabled:opacity-50"
          >
        ...
        <button
          ...
          className="px-3 py-1.5 rounded-lg bg-[#f5a623] text-[#0a1628] text-xs font-medium hover:bg-[#f5a623]/90 transition-colors flex items-center gap-1"
        >
          <svg
            width="12"
            height="12"
