...
export function FeaturedArtists() {
  ...
  useEffect(() => {
    ...
    fetch('/api/artists/featured')
      ...
      .catch(() => {
        if (!cancelled) setError('Failed to load featured artists');
      })
      .finally(() => {
  ...
  return (
    <section>
      ...
      {expandedUsername && (
        <div className="mt-4">
          <ArtistSpotlight username={expandedUsername} onClose={() => setExpandedUsername(null)} />
        </div>
      )}
    </section>
  );
    ...
    <section>
      ...
      <div className="flex gap-3 overflow-hidden">
        {Array.from({ length: 4 }).map((_, i) => (
          <div
            key={i}
            className="flex-shrink-0 w-[140px] rounded-xl overflow-hidden bg-white/5 animate-pulse"
          >
