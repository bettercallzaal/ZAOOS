  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {playlists.map((pl, i) => {
          const isActive = radio.isRadioMode && radio.currentStationIndex === i;
          ...
          return (
            ...
            >
              {/* Playlist artwork */}
              <div
                className={`w-14 h-14 flex-shrink-0 rounded-lg flex items-center justify-center overflow-hidden ${
                  isActive
                    ? 'bg-[#f5a623]/20 ring-1 ring-[#f5a623]/30'
                    : 'bg-gradient-to-br from-[#1a2a3a] to-[#0a1628]'
                }`}
              >
                {isActive ? <PlayingBars /> : <MusicIcon className="w-6 h-6 text-[#f5a623]/40" />}
              </div>
