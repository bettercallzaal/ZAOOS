...
export function ShareToChatButton({
  ...
  return (
    ...
    >
      {status === 'sending' ? (
        <div
          className={`${compact ? 'w-3.5 h-3.5' : 'w-4 h-4'} border-2 border-gray-600 border-t-[#f5a623] rounded-full animate-spin`}
        />
      ) : status === 'success' ? (
        <svg
          className={`${compact ? 'w-3.5 h-3.5' : 'w-4 h-4'} text-green-400`}
          viewBox="0 0 24 24"
          fill="none"
          stroke="currentColor"
          strokeWidth={2}
