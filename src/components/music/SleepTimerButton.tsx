    ...
    <div ref={ref} className={`relative ${className}`}>
      ...
      <button
        ...
        className={`relative flex items-center justify-center transition-colors ${
          compact ? 'p-1.5 rounded' : 'p-1.5 rounded-lg hover:bg-white/5'
        } ${isActive ? 'text-[#f5a623]' : 'text-gray-400 hover:text-white'}`}
        ...
        >
          ...
          {isActive && activeTimer !== 'endOfTrack' && remainingMs > 0 && (
            <div className="px-3 py-2 border-t border-white/[0.08]">
              <p className="text-[10px] text-gray-500 text-center">Pausing in {badgeText}</p>
            </div>
          )}
          {isActive && activeTimer === 'endOfTrack' && (
            <div className="px-3 py-2 border-t border-white/[0.08]">
              <p className="text-[10px] text-gray-500 text-center">Pausing after current track</p>
            </div>
          )}
        </div>
      )}
