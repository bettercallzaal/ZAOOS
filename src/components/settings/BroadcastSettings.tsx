  ...
  return (
    <div className="bg-[#0d1b2a] rounded-xl border border-white/[0.08] p-6">
      ...
      <p className="text-gray-500 text-sm mb-4">
        Save your stream keys so you can go live with one click
      </p>
      ...
      {loading ? (
        <div className="space-y-2">
          {[1, 2].map((i) => (
            <div key={i} className="h-14 bg-[#0a1628] rounded-lg animate-pulse" />
          ))}
        </div>
      ) : targets.length === 0 ? (
        ...
        <div className="space-y-2">
          {targets.map((target) => {
            ...
            return (
              <div
                key={target.id}
                className="flex items-center justify-between bg-[#0a1628] border border-white/[0.08] rounded-lg px-4 py-3"
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-gray-500 text-xs">
                      {platform?.label || target.platform} · ••••{target.stream_key.slice(-4)}
                    </div>
                  </div>
                </div>
                <button
