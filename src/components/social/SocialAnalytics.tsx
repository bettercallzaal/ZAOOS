...
function ScoreBar({ score, max = 1 }: { score: number; max?: number }) {
  ...
  return (
    <div className="w-full h-2 bg-gray-800 rounded-full overflow-hidden">
      <div
        className="h-full rounded-full bg-gradient-to-r from-[#f5a623] to-[#f5c623]"
        style={{ width: `${pct}%` }}
      />
    </div>
  );
...
export function SocialAnalytics({ currentFid }: { currentFid: number }) {
  ...
  const computeStats = useCallback(
    (followers: FollowerUser[], profile: { follower_count: number; following_count: number }) => {
      ...
      const mutuals = followers.filter(
        (u) => u.viewer_context?.following && u.viewer_context?.followed_by,
      );
      const pb = followers.filter((u) => u.power_badge);
      const scores = followers
        .map((u) => u.experimental?.neynar_user_score ?? 0)
        .filter((s) => s > 0);
      const avgScore =
        scores.length > 0
      ...
      setFollowingFids(
        new Set(followers.filter((u) => u.viewer_context?.following).map((u) => u.fid)),
      );
      setStats({
        followerCount: fc || 0,
        ...
        ratio,
        zaoCount: zao.length,
        zaoPercent: followers.length > 0 ? Math.round((zao.length / followers.length) * 100) : 0,
        mutualCount: mutuals.length,
        powerBadgePercent:
          followers.length > 0 ? Math.round((pb.length / followers.length) * 100) : 0,
        avgScore,
      });
      setTopFollowers(
        [...followers]
          .filter((u) => (u.experimental?.neynar_user_score ?? 0) > 0)
          .sort(
            (a, b) =>
              (b.experimental?.neynar_user_score ?? 0) - (a.experimental?.neynar_user_score ?? 0),
          )
          .slice(0, 5),
      );
    },
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
          {loading ? (
            ...
            <>
              ...
              <StatCard
                label="ZAO Members"
                value={`${stats.zaoCount} (${stats.zaoPercent}%)`}
                accent
              />
              <StatCard label="Mutual Follows" value={String(stats.mutualCount)} />
            </>
          ) : (
            <p className="col-span-2 md:col-span-4 text-center py-6 text-sm text-gray-500">
              Unable to load network stats
            </p>
          )}
        </div>
      </div>
        ...
        {loading ? (
          ...
          <div className="bg-[#0d1b2a] rounded-xl p-4 border border-white/[0.08] space-y-4">
            ...
            <div className="flex justify-between items-baseline pt-1 border-t border-white/[0.08]">
              ...
              <span
                className={`text-sm font-semibold ${stats.ratio >= 1 ? 'text-green-400' : 'text-gray-300'}`}
              >
                {stats.ratio}x
              </span>
            </div>
        ...
        {growthLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
            {(
              [
                {
                  label: 'Followers',
                  data: growth.followers,
                  value: growth.currentFollowers,
                  change: growth.followerChange,
                  color: '#f5a623',
                },
                {
                  label: 'Following',
                  data: growth.following,
                  value: growth.currentFollowing,
                  change: growth.followingChange,
                  color: '#60a5fa',
                },
                {
                  label: 'Engagement',
                  data: growth.engagement,
                  value: growth.currentEngagement,
                  change: growth.engagementChange,
                  color: '#a78bfa',
                },
              ] as const
            ).map((metric) => (
              ...
              >
                ...
                <div className="flex items-baseline justify-between mt-2">
                  ...
                  <span
                    className={`text-xs font-medium ${metric.change > 0 ? 'text-green-400' : metric.change < 0 ? 'text-red-400' : 'text-gray-500'}`}
                  >
                    {metric.change > 0 ? '+' : ''}
                    {metric.change}
                  </span>
          ...
          <div className="bg-[#0d1b2a] rounded-xl border border-white/[0.08] px-4 py-8 text-center">
            ...
            <p className="text-[11px] text-gray-600 mt-1">
              Daily snapshots will build your trend charts over time
            </p>
          </div>
        )}
      </div>
      ...
      <div>
        ...
        {unfollowersLoading ? (
            ...
            {unfollowers.slice(0, 8).map((u) => (
              <div key={u.fid} className="flex items-center gap-3 px-3 py-2.5">
                ...
                <span className="text-[10px] text-red-400/70 flex-shrink-0">
                  {u.unfollowed_at
                    ? new Date(u.unfollowed_at).toLocaleDateString('en-US', {
                        month: 'short',
                        day: 'numeric',
                      })
                    : 'Recently'}
          ...
          <div className="bg-[#0d1b2a] rounded-xl border border-white/[0.08] px-4 py-8 text-center">
            ...
            <p className="text-[11px] text-gray-600 mt-1">
              Unfollowers are tracked with daily snapshots of your network
            </p>
          </div>
        )}
      </div>
      ...
      <div>
        ...
        {loading ? (
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    ...
                    {!isFollowed && user.fid !== currentFid && (
                      <button
                        ...
                        disabled={followLoading === user.fid}
                    ...
                    {isFollowed && user.fid !== currentFid && (
                      <span className="text-[10px] text-green-400/70">Following</span>
                    )}
                  </div>
                </div>
              );
          ...
          <div className="bg-[#0d1b2a] rounded-xl border border-white/[0.08] px-4 py-8 text-center">
            ...
            <p className="text-[11px] text-gray-600 mt-1">
              Scores will appear as followers interact with the network
            </p>
          </div>
        )}
      </div>
            ...
            <div className="space-y-1">
              {channelRanks.map((entry, i) => {
                ...
                return (
                  ...
                  >
                    <span
                      className={`w-5 text-right text-xs tabular-nums ${isMe ? 'text-[#f5a623] font-bold' : 'text-gray-600'}`}
                    >
                      {i + 1}.
                    </span>
                    <Avatar
                      src={entry.pfp_url}
                      alt={`${entry.display_name || entry.username} avatar`}
                      small
                    />
                    <span
                      className={`font-medium truncate flex-1 ${isMe ? 'text-[#f5a623]' : 'text-white'}`}
                    >
                      {entry.display_name}
                    ...
                    <div className="flex items-center gap-1.5 flex-shrink-0">
                      ...
                      {i === 0 && (
                        ...
                        >
                          <path
                            fillRule="evenodd"
                            d="M10 1l2.928 6.856L20 8.59l-5.072 4.574L16.18 20 10 16.29 3.82 20l1.252-6.836L0 8.59l7.072-.734L10 1z"
                          />
                        </svg>
                      )}
                    </div>
