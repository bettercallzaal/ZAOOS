...
import { useCallStateHooks, hasAudio, hasVideo, ParticipantsAudio, Video, StreamVideoParticipant } from '@stream-io/video-react-sdk';
      ...
      {speakers.length > 0 && (
        <div className="mb-6">
          ...
          <div className="grid grid-cols-4 md:grid-cols-5 gap-3">
            {speakers.map((p) => (
              <SpeakerItem key={p.sessionId} participant={p} />
            ))}
          </div>
        </div>
      )}
      ...
      {listeners.length > 0 && (
        <div>
          ...
          <div className="grid grid-cols-5 md:grid-cols-6 gap-3">
            {listeners.map((p) => (
              <ListenerItem key={p.sessionId} participant={p} />
            ))}
          </div>
        </div>
      )}
...
const SpeakerItem = memo(function SpeakerItem({ participant: p }: { participant: StreamVideoParticipant }) {
  return (
...
const ListenerItem = memo(function ListenerItem({ participant: p }: { participant: StreamVideoParticipant }) {
  return (
    <div className="flex flex-col items-center">
      {hasVideo(p) ? (
