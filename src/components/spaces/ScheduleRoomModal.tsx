...
export function ScheduleRoomModal({ isOpen, onClose, onCreated }: ScheduleRoomModalProps) {
  ...
  return (
    ...
    >
      <div
        className="bg-[#0d1b2a] border border-white/[0.08] rounded-t-2xl sm:rounded-2xl p-6 w-full max-w-md max-h-[90dvh] overflow-y-auto"
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full bg-[#0a1628] border border-white/[0.08] rounded-xl px-4 py-2.5 text-white text-sm focus:border-[#f5a623] focus:outline-none transition-colors"
              disabled={loading}
            />
          </div>
          ...
          <div>
            ...
            <input
              ...
              min={minDate}
              className="w-full bg-[#0a1628] border border-white/[0.08] rounded-xl px-4 py-2.5 text-white text-sm focus:border-[#f5a623] focus:outline-none transition-colors [color-scheme:dark]"
              disabled={loading}
            />
          </div>
            ...
            <div className="flex flex-wrap gap-2">
              {CATEGORIES.map((c) => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-colors ${category === c.id ? 'border-[#f5a623]/60 bg-[#f5a623]/10 text-[#f5a623]' : 'border-white/[0.08] text-gray-500 hover:border-gray-600'}`}
                  disabled={loading}
                >
                  {c.label}
                </button>
