...
export function ScheduleRoomModal({ isOpen, onClose, onCreated }: ScheduleRoomModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-sm px-0 sm:px-4" onKeyDown={(e) => e.key === 'Escape' && onClose()} onClick={onClose}>
      <div className="bg-[#0d1b2a] border border-white/[0.08] rounded-t-2xl sm:rounded-2xl p-6 w-full max-w-md max-h-[90dvh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} maxLength={100} placeholder="What's this space about?" className="w-full bg-[#0a1628] border border-white/[0.08] rounded-xl px-4 py-2.5 text-white text-sm focus:border-[#f5a623] focus:outline-none transition-colors" disabled={loading} />
          </div>

          <div>
            <label className="text-gray-400 text-xs font-medium uppercase tracking-wider mb-1.5 block">Description</label>
            <textarea value={description} onChange={(e) => setDescription(e.target.value)} maxLength={500} rows={2} placeholder="Tell people what to expect..." className="w-full bg-[#0a1628] border border-white/[0.08] rounded-xl px-4 py-2.5 text-white text-sm focus:border-[#f5a623] focus:outline-none transition-colors resize-none" disabled={loading} />
          </div>

          <div>
            <label className="text-gray-400 text-xs font-medium uppercase tracking-wider mb-1.5 block">When *</label>
            <input type="datetime-local" value={scheduledAt} onChange={(e) => setScheduledAt(e.target.value)} min={minDate} className="w-full bg-[#0a1628] border border-white/[0.08] rounded-xl px-4 py-2.5 text-white text-sm focus:border-[#f5a623] focus:outline-none transition-colors [color-scheme:dark]" disabled={loading} />
          </div>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {CATEGORIES.map((c) => (
                <button key={c.id} type="button" onClick={() => setCategory(c.id)} className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-colors ${category === c.id ? 'border-[#f5a623]/60 bg-[#f5a623]/10 text-[#f5a623]' : 'border-white/[0.08] text-gray-500 hover:border-gray-600'}`} disabled={loading}>
                  {c.label}
                </button>
              ))}
            </div>
          ...
          <div className="flex gap-3 pt-2">
            <button type="button" onClick={onClose} className="flex-1 px-4 py-2.5 bg-gray-800/50 text-gray-300 rounded-xl text-sm hover:bg-gray-800 transition-colors border border-white/[0.08]" disabled={loading}>Cancel</button>
