...
export function TwitchInteractivePanel() {
  ...
  const inputCls =
    'w-full bg-[#0a1628] border border-white/[0.08] rounded-lg px-3 py-2 text-white text-sm focus:border-[#9146ff] focus:outline-none placeholder:text-gray-600';
  const btnPrimary =
        ...
        (activePoll ? (
          <div className="text-center space-y-3">
            ...
            <button
              onClick={endPoll}
              disabled={loading}
              className={`${btnPrimary} bg-red-600 text-white hover:bg-red-500 w-full`}
            >
          ...
          <div className="space-y-3">
            ...
            {pollChoices.length < 5 && (
              <button
                ...
                className="text-[#9146ff] text-xs hover:underline"
              >
                + Add choice
              </button>
            )}
            <div className="flex gap-1.5 flex-wrap">
              {DURATION_OPTIONS.map((d) => (
                <button
                  ...
                  className={`px-3 py-1 rounded-md text-xs transition-colors ${pollDuration === d.value ? 'bg-[#9146ff] text-white' : 'bg-[#0a1628] text-gray-400 border border-white/[0.08]'}`}
                >
                  {d.label}
                </button>
              ))}
            ...
            <button
              onClick={startPoll}
              disabled={loading}
              className={`${btnPrimary} bg-[#9146ff] text-white hover:bg-[#7c3aed] w-full`}
            >
        ...
        (activePred ? (
          <div className="space-y-3">
            <p className="text-purple-300 text-sm font-medium text-center">
              Active: {activePred.title}
            </p>
            <p className="text-gray-500 text-xs text-center">Pick the winner:</p>
            {activePred.outcomes.map((o) => (
              <button
                key={o.id}
                onClick={() => resolvePrediction(o.id)}
                disabled={loading}
                className={`${btnPrimary} w-full bg-[#0a1628] border border-[#9146ff]/40 text-purple-300 hover:bg-[#9146ff]/20`}
              >
                {o.title}
              </button>
            ))}
          ...
          <div className="space-y-3">
            ...
            {predOutcomes.map((o, i) => (
              <input
                ...
                value={o}
                onChange={(e) => {
                  const next = [...predOutcomes];
                  next[i] = e.target.value;
                  setPredOutcomes(next);
                }}
            ...
            <div className="flex gap-1.5 flex-wrap">
              {DURATION_OPTIONS.map((d) => (
                <button
                  key={d.value}
                  onClick={() => setPredDuration(d.value)}
                  className={`px-3 py-1 rounded-md text-xs transition-colors ${predDuration === d.value ? 'bg-[#9146ff] text-white' : 'bg-[#0a1628] text-gray-400 border border-white/[0.08]'}`}
                >
                  {d.label}
                </button>
              ))}
        ...
        <div className="space-y-3">
            ...
            <div className="space-y-2 max-h-40 overflow-y-auto">
              {clips.map((c) => (
                <a
                  ...
                  className="block bg-[#0a1628] border border-white/[0.08] rounded-lg px-3 py-2 text-sm text-purple-300 hover:border-[#9146ff]/50 truncate"
                >
          ...
          {clips.length === 0 && (
            <p className="text-gray-500 text-xs text-center">
              No clips yet — create one while live
            </p>
          )}
        </div>
      )}
