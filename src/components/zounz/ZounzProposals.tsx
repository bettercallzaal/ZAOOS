...
export default function ZounzProposals() {
  ...
  const filtered = proposals.filter((p) => {
    ...
    if (filter === 'passed')
      return p.status === 'Succeeded' || p.status === 'Executed' || p.status === 'Queued';
    if (filter === 'failed')
      return p.status === 'Defeated' || p.status === 'Expired' || p.status === 'Canceled';
    return true;
  });
  ...
  const countForFilter = (f: FilterTab): number => {
    ...
    if (f === 'active')
      return proposals.filter((p) => p.status === 'Active' || p.status === 'Pending').length;
    if (f === 'passed')
      return proposals.filter(
        (p) => p.status === 'Succeeded' || p.status === 'Executed' || p.status === 'Queued',
      ).length;
    if (f === 'failed')
      return proposals.filter(
        (p) => p.status === 'Defeated' || p.status === 'Expired' || p.status === 'Canceled',
      ).length;
    return 0;
  };
    ...
    <div className="rounded-2xl border border-[#f5a623]/20 bg-[#0d1b2a] p-4 space-y-4">
      ...
      {!isConnected && (
        <div className="bg-[#f5a623]/5 border border-[#f5a623]/10 rounded-xl p-3 text-center">
          <p className="text-sm text-gray-400">Connect wallet to see your ZOUNZ voting power</p>
        </div>
      )}
      {isConnected && votes > 0 && (
        <div className="bg-[#f5a623]/5 border border-[#f5a623]/10 rounded-xl p-3 text-center">
          <p className="text-sm text-[#f5a623]">
            You have <span className="font-bold">{votes}</span> vote{votes !== 1 ? 's' : ''} in
            ZOUNZ governance
          </p>
        </div>
      )}
      ...
      {loadingProposals ? (
        <div className="space-y-2">
          {[1, 2, 3].map((i) => (
            <div
              key={i}
              className="bg-[#0a1628] rounded-lg border border-white/[0.08] p-4 animate-pulse"
            >
        ...
        <div className="space-y-2">
          {filtered.map((proposal) => (
            <ZounzProposalCard key={proposal.proposalId} proposal={proposal} votingPower={votes} />
          ))}
        </div>
      )}
      ...
      <p className="text-xs text-gray-500 text-center">
        ZOUNZ NFT holders vote on-chain. Proposals execute trustlessly via the Governor contract on
        Base.
      </p>
