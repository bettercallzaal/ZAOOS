...
export function useGroups(
  ...
  findClientForConversation: (
    convId: string,
  ) => Promise<{ client: AnyClient; address: string } | null>,
        ...
        if (profiledMembers.length > 0) {
          try {
            ...
            for (const xm of xmtpMembers) {
              const addresses = (xm.accountIdentifiers ?? []).map((id: { identifier: string }) =>
    ...
    async (conversationId: string) => {
      ...
      try {
        ...
        return xmtpMembers.map((m) => {
          ...
          const addresses = (m.accountIdentifiers ?? []).map((id: { identifier: string }) =>
            id.identifier.toLowerCase(),
          );
