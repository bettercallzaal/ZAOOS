...
export function useMessages(
   ...
   */
  const startGlobalStreams = useCallback(
    async (client: AnyClient) => {
      ...
      try {
          ...
          onValue: (msg: DecodedMessage) => {
            ...
            if (isForActiveConv && !isLoadingConvRef.current) {
              ...
              const senderName = isFromMe
                ? undefined
                : mp?.displayName ||
                  (mp?.username ? `@${mp.username}` : undefined) ||
                  peer?.displayName ||
        ...
        setError((prev) => {
          if (
            prev &&
            (prev.includes('stream') || prev.includes('disconnected') || prev.includes('reconnect'))
          ) {
            return null;
          }
          return prev;
        ...
        if (reconnectAttemptsRef.current >= MAX_RECONNECT_ATTEMPTS) {
          setReconnecting(false);
          setError(
            'Message stream disconnected after multiple retries. Use the reconnect button to try again.',
          );
          return;
        }

        // Auto-reconnect with exponential backoff, capped at 30s
        const delay = Math.min(5000 * 1.5 ** (reconnectAttemptsRef.current - 1), 30000);
        setReconnecting(true);
        setError(
          `Stream disconnected. Reconnecting (attempt ${reconnectAttemptsRef.current}/${MAX_RECONNECT_ATTEMPTS})...`,
        );
  ...
  const sendMessage = useCallback(
    async (text: string) => {
      ...
      try {
        ...
        setMessages((prev) => prev.map((m) => (m.id === msgId ? { ...m, sendFailed: true } : m)));
