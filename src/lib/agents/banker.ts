...
/**
 * BANKER agent daily routine.
 * Called by Vercel cron at 2 PM UTC.
 * Determines today's action, checks budget, executes.
 */
export async function runBanker(): Promise<{
  action: AgentAction;
  status: 'success' | 'failed' | 'skipped';
  details: string;
}> {
  const config = await getAgentConfig('BANKER');
  if (!config) {
    return { action: 'report', status: 'failed', details: 'No config found for BANKER' };
  }
  ...
  const action = BANKER_SCHEDULE[dayOfWeek] || 'report';

  // Check daily budget
  const spent = await getDailySpend('BANKER');
  ...
  try {
    ...
    switch (action) {
      case 'buy_zabal': {
        ...
        const hash = await executeSwap('BANKER', quote);
        await burnZabal('BANKER', BigInt(quote.buyAmount));
        ...
        await postTradeUpdate({ agentName: 'BANKER', action: 'buy_zabal', details, txHash: hash });

        logger.info(`[BANKER] buy_zabal: $${tradeUsd.toFixed(2)} -> ${quote.buyAmount} ZABAL`);
      ...
      case 'buy_sang': {
        ...
        await postTradeUpdate({ agentName: 'BANKER', action: 'buy_sang', details, txHash: hash });

        logger.info(`[BANKER] buy_sang: $${tradeUsd.toFixed(2)} -> ${quote.buyAmount} SANG`);
        return { action, status: 'success', details };
      }
      ...
      case 'report': {
        ...
        await postTradeUpdate({ agentName: 'BANKER', action: 'report', details });
        return { action, status: 'success', details };
      }
    ...
    logger.error(`[BANKER] ${action} failed:`, message);
