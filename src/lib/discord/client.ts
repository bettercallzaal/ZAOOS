 ...
 */
export async function getChannelMessages(channelId: string, limit = 50): Promise<APIMessage[]> {
  const client = getClient();
  if (!client) return [];

  try {
    const messages = (await client.get(Routes.channelMessages(channelId), {
...
export async function getGuildMembers(limit = 100): Promise<APIGuildMember[]> {
  ...
  try {
    const members = (await client.get(Routes.guildMembers(DISCORD_GUILD_ID), {
      query: new URLSearchParams({ limit: String(limit) }),
    })) as APIGuildMember[];
    return members;
  } catch (err) {
...
export async function getActiveThreads(): Promise<APIThreadChannel[]> {
  ...
  try {
    const result = (await client.get(Routes.guildActiveThreads(DISCORD_GUILD_ID))) as {
 ...
 */
export async function getThreadMessages(threadId: string, limit = 50): Promise<APIMessage[]> {
