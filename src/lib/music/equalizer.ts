...
export class AudioEqualizer {
  ...
  connectIfActive() {
    if (this.isActive() && !this.activeElement) {
      ...
      const audioA = (globalThis as Record<string, unknown>).__zao_audio_a as
        | HTMLAudioElement
        | undefined;
      const audioB = (globalThis as Record<string, unknown>).__zao_audio_b as
        | HTMLAudioElement
        | undefined;
  ...
  connect(element: HTMLAudioElement) {
    ...
    if (this.activeElement) {
      ...
      if (prevSource) {
        ...
        try {
          prevSource.connect(this.context!.destination);
        } catch {}
      }
    }
