...
import type { AudioController, TrackMetadata, TrackType } from '@/types/music';
...
export function PlayerProvider({ children }: { children: ReactNode }) {
  ...
  const [restoredTrack, setRestoredTrack] = useState<{
    ...
    return _persisted.metadata
      ? {
          metadata: _persisted.metadata,
          position: _persisted.position ?? 0,
          duration: _persisted.duration ?? 0,
        }
      : null;
  });
    ...
    const handler = (e: KeyboardEvent) => {
      ...
      switch (e.key) {
        case ' ':
          ...
          break;
        case 'ArrowRight':
          if (e.shiftKey && onEndedRef.current) {
            onEndedRef.current();
          } // Shift+Right = next track
          else if (state.metadata) {
            ctrl?.seek(state.position + 10000);
            dispatch({ type: 'SEEK', payload: state.position + 10000 });
          }
          break;
        case 'ArrowLeft':
          if (state.metadata) {
            ctrl?.seek(Math.max(0, state.position - 10000));
            dispatch({ type: 'SEEK', payload: Math.max(0, state.position - 10000) });
          }
          break;
        case 'ArrowUp':
          if (e.shiftKey) {
  ...
  const registerController = useCallback((type: TrackType, controller: AudioController) => {
